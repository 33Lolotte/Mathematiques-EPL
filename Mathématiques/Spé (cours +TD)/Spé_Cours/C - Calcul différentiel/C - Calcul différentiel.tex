...
\addto\captionsfrench{\renewcommand*{\contentsname}{Calcul différentiel}}
...
\begin{itemize}
	\item Lorsque l'on fixe une base $\mathcal{B}=(e_1,\ldots,e_p)$ de $E$, on sait que l'application $\Phi$ définie sur $\R^p$ par $\Phi(x_1,\ldots,x_p)=\displaystyle\sum_{i=1}^{p}x_ie_i$ est un isomorphisme de $\R^p$ sur $E$.\\
	...
	$$\forall i\in\llbracket1,n\rrbracket,\ \forall x\in\Omega, f(x)=\sum_{i=1}^{n}f_i(x)e_i'.$$
...
\begin{dfn}
	...
	$$\frac{\partial f}{\partial x_j}(a)\quad\quad\text{ou}\quad\quad \partial_jf(a)$$ est, lorsqu'elle existe, la dérivée de $f$ en $a$ selon le vecteur $e_j$.
\end{dfn}
...
\begin{rmq}
	\begin{itemize}
		...
		Sauf mention expresse du contraire, lorsque l'on parle des dérivées partielles pour une fonction définie sur un ouvert de $\R^p$, il est sous-entendu que c'est par rapport à la base canonique.
		...
		\item Si $f:\Omega\to F$ est définie par une expression où les variables sont notées $u_1,\ldots,u_p$, on notera, si elles existent, $\displaystyle\frac{\partial f}{\partial u_1},\ldots,\frac{\partial f}{\partial u_p}$ ses dérivées partielles.
		\item En particulier, lorsque $E=\R^2$ ou $E=\R^3$, on a l'habitude de noter $(x,y)$ ou $(x,y,z)$ les variables. Les dérivées partielles seront donc traditionnellement notées $\displaystyle\frac{\partial f}{\partial x},\frac{\partial f}{\partial y}$, et éventuellement $\displaystyle\frac{\partial f}{\partial z}$.
	\end{itemize}
\end{rmq}
...
\begin{prop}[Différentiabilité et continuité]%prop
...
\begin{notation}
	\begin{itemize}
		...
		\item Le développement limité de $f$ en $a$ à l'ordre $1$ s'écrit donc, si elle existe :
...
\begin{prop}[Cas particulier des fonctions d'une seule variable]%prop
	Supposons que $\Omega$ soit un intervalle ouvert de $E=\R$.\\
	Une application $f:\Omega\to F$ est différentiable en $a\in\Omega$ si, et seulement si, elle est dérivable en $a$, et alors $\d f(a)$ est l'application linéaire $h\mapsto hf'(a)$.\\
...
\begin{prop}[Différentielle d'une application linéaire]%prop
...
Soit $\mathcal{B}=(e_1,\ldots,e_p)$ une base de $E$ et $\mathcal{B}'=(e_1',\ldots,e_n')$ une base de $F$. Soit $f:\Omega\to F$ une fonction différentiable en $a\in\Omega$ ; notons $f_1,\ldots,f_n$ les fonctions coordonnées de $f$ dans la base $\mathcal{B}'$. On sait qu'une fonction de la variable réelle à valeurs dans un espace vectoriel de dimension finie est dérivable si, et seulement si, toutes ses fonctions coordonnées sont dérivables.\\
Ainsi, pour tout $j\in\llbracket1,p\rrbracket$, puisque $\partial_jf(a)$ est définie, pour tout $i\in\llbracket1,n\rrbracket$, les dérivées partielles $\partial_jf_i(a)$ sont définies et $\partial_jf(a)=\displaystyle\sum_{i=1}^{n}\partial_jf_i(a)e_i'=\sum_{i=1}^{n}\frac{\partial f_i}{\partial x_j}(a)e_i'$.
...
\begin{prop}[Composantes du gradient en base orthonormée]%prop
...
\begin{proof}
	...
	\begin{align*}
		M(f_1(a+h),\ldots,f_q(a+h))&=M(f_1(a),\ldots,f_q(a))\\&+\sum_{k=1}^{q}M(f_1(a),\ldots,f_{k-1}(a),\d f_k(a)\cdot h),f_{k+1}(a),\ldots,f_q(a)+o(h)
	\end{align*}
...
\begin{proof}
	\begin{itemize}
		...
		\item Fixons une base $\mathcal{B}=(e_1,\ldots,e_p)$ de $E$ et une base $\mathcal{C}$ de $F$. Au moyen de l'isomorphisme $\text{Mat}_{\mathcal{B},\mathcal{C}}$ entre $\mathcal{L}(E,F)$ et $\mathcal{M}_{n,p}(\R)$, il suffit de montrer que l'application $a\mapsto\text{Mat}_{\mathcal{B},\mathcal{C}}(u(a))$ est continue, c'est-à-dire que toutes ses fonctions composantes sont continues. Or, le coefficient d'indice $(i,j)$ de la matrice de $u(a)$ est $\varphi_i(u(a)\cdot e_j)$, où $\varphi_i$ est la $i$-ème forme linéaire coordonnée dans la base $\mathcal{C}$ dont la continuité, ainsi que celle de $a\mapsto u(a)\cdot e_j$ par hypothèse, donne le résultat.
	\end{itemize}
\end{proof}

\begin{cor}%cor
	Si $f_1,\ldots,f_q$ sont différentiables (respectivement de classe $\mathcal{C}^1$), alors $M(f_1,\ldots,f_q)$ est différentiable (respectivement de classe $\mathcal{C}^1$).
\end{cor}
\begin{proof}
...
\begin{ex}
	...
	$$\d(f_1\times\cdots\times f_q)(a)=\sum_{k=1}^{q}f_1(a)\times\cdots\times f_{k-1}(a)\times f_{k+1}(a)\times\cdots\times f_q(a)\d f_k(a).$$
	Si $E$ est euclidien, cela peut se réécrire :
	$$\nabla(f_1\times\cdots\times f_q)(a)=\sum_{k=1}^{q}f_1(a)\times\cdots\times f_{k-1}(a)\times f_{k+1}(a)\times\cdots\times f_q(a)\nabla f_k(a).$$
\end{ex}
...
\begin{cor}%cor
	...
	\begin{itemize}
		\item Si $f_1$ et $f_2$ sont différentiables en $a\in\Omega$, alors $B(f_1,f_2)$ est différentiable en $a$ et :
...
\begin{prop}[Différentielle d'une composée]%prop
...
\begin{prop}[Matrice jacobienne d'une composée]
...
\begin{ex}
	\begin{enumerate}
		...
		Soit $L\in\mathcal{L}(F,G)$ et $f:\Omega\to F$. L'application linéaire $L$ est de classe $\mathcal{C}^1$ et $\forall b\in F,\ dL(b)=L$. Donc, par composition :
		\begin{itemize}
...
\begin{prop}[Règle de la chaîne]%prop
...
\begin{dfn}
	Soit $f$ une bijection de l'ouvert $U\subset E$ sur l'ouvert $V\subset F$. On dit que $f$ est un \textbf{difféomorphisme de $U$ sur $V$} si $f$ et $f\inv$ sont différentiables.
\end{dfn}
...
\begin{rmq}
	Plus généralement, l'assertion "s'il existe un homéomorphisme entre $U$ un ouvert de $E$ et $V$ un ouvert de $F$ (où $E$ et $F$ sont de dimension finie), alors $\dim(E)=\dim(F)$" est également vraie, mais la démonstration est beaucoup plus délicate.
\end{rmq}
...
\begin{prop}[Caractérisation des fonctions constantes sur un connexe]%prop
...
Dans le cas où $E$ est euclidien, la conclusion du théorème précédent peut se formuler ainsi : si $\nabla g(a)\ne0$, on a $T_aX=(\nabla g(a))^\bot$.
...
\begin{prop}%prop
	Si $f\in\mathcal{C}^k(\Omega,F)$, alors $f$ est de classe $\mathcal{C}^i$, pour tout $i\in\llbracket0,k\rrbracket$.
\end{prop}
...
\begin{thm}[Théorème de Schwarz]%thm
	Soit $f:\Omega\to F$ une fonction de classe $\mathcal{C}^2$. On a alors :
...
"N'importe quelle canne à pêche permet d'attraper un tel poisson." [À propos de l'exercice qui précède...]
...
\end{ex}
%termes de degré $1$ donnent l'extremum
