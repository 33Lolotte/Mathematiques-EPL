...
\end{ex}
\begin{proof}
	Effectuer la transvection par blocs $L_2\leftarrow L_2-AL_1$ pour se ramener à une matrice triangulaire par blocs de blocs diagonaux $I_n$ et $-A^2$. Un exemple du cours (obtenu par transvection par blocs avec un bloc diagonal inversible) donne alors le résultat souhaité.
\end{proof}
...
\begin{ex}[Moulin.Mat.30]{$\star$ Une application combinatoire du rang}
	\begin{enumerate}
		...
		\item Soit $m$, $n$ et $r$ trois entiers strictement positifs, $E=\{a_1,\ldots,a_m\}$ un ensemble à $m$ éléments et $(A_j)_{1\leqslant j\leqslant n}$ une famille de $n$ parties de $E$ deux à deux distinctes. Écrire la matrice $M=(\card(A_i\cap A_j))\in\mathcal{M}_n(\R)$ sous la forme $B^TB$ où $B$ est une matrice de $\mathcal{M}_{m,n}(\R)$ à préciser.
...
\begin{ex}[Châteaux]{Transvections et dilatations}
	...
	\end{enumerate}
\end{ex}
\begin{ex}[Châteaux]{Une application des transvections}
...
\end{ex}
\begin{proof}
	Prendre $e_1$ un vecteur non nul de l'image. Fixer $e_2$ un antécédent de $e_1$, puis prendre $e_3$ dans le noyau de $u$ non colinéaire à $e_2$. Montrer que l'ensemble de ces $3$ vecteurs forme une base en utilisant l'hypothèse $u^2=0$, et vérifier que la matrice de $u$ dans cette base est bien de la forme souhaitée.
\end{proof}
...
\begin{ex}[Moulin.Mat.37]{}
	...
	\begin{enumerate}
		...
		0&B
		\end{pmatrix},$$
		$$\varphi_1:M\in\mathcal{M}_{n+p}(\K)\mapsto \begin{pmatrix}
			A&C\\
			0&B
		\end{pmatrix}M-M\begin{pmatrix}
			A&0\\
...
\begin{ex}[Moulin.Mat.39]{$\Delta$ Formes linéaires sur $\mathcal{M}_n(\K)$}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Regarder sur les matrices élémentaires. Ce sont les formes linéaires proportionnelles à la trace.
	\end{enumerate}
\end{proof}

\begin{ex}[Châteaux]{Formes linéaires sur $\mathcal{M}_n(\K)$ et applications}
	\begin{enumerate}
...
\begin{ex}[Moulin.Mat.48]{}
	Montrer que tout endomorphisme de l'algèbre $\mathcal{M}_n(\K)$ conserve la trace.
\end{ex}
\begin{proof}
	Soit $u$ un endomorphisme de $\mathcal{M}_n(\K)$. On pose $\varphi=\Tr\circ u$ qui est immédiatement une forme linéaire. Alors pour tout $(A,B)\in\mathcal{M}_n(\K)^2$, on a : $$\varphi(AB)=\Tr(u(AB))=\Tr(u(A)u(B))=\Tr(u(B)u(A))=\Tr(u(BA))=\varphi(BA)$$ D'après un exercice précédent, il existe $\lambda\in\K$ tel que $\varphi=\lambda\Tr$. En évaluant en $I_n$, on obtient $\lambda=1$ si bien que $\Tr\circ u=\Tr$ donc $u$ conserve la trace.
\end{proof}
...
\begin{ex}[]{}
	Soit $A$ et $B$ deux matrices de de $\mathcal{M}_n(\R)$ telles que : $$\forall k\in\llbracket0,2n\rrbracket,\ A+kB\in\mathcal{GL}_n(\Z)$$ Calculer $\det(A)$ et $\det(B)$.
\end{ex}
\begin{proof}
	On considère $P=\det(A+XB)\in\R_n[X]$. D'après l'exercice précédent, on a : $$\forall k\in\llbracket0,2n\rrbracket,\ P(k)=\pm1$$ Mais comme $P$ est de degré $n$ et que $P$ vaut au moins $n+1$ fois $1$ ou $-1$, alors $P$ est constant, égal à $1$ ou $-1$. Donc $\det(A)$ vaut $1$ ou $-1$. Et ensuite : $$\forall x\in\R\st,\ P(x)=x^n\det\left(\frac{1}{x}A+B\right)=\text{Cte}$$ En faisant tendre $x$ vers $+\infty$, par continuité du déterminant, on a $\det(B)=0$.
\end{proof}
...
\begin{ex}{Théorème de Bézout généralisé}
	Soit $A$ et $B$ dans $\mathcal{M}_n(\Z)$. On suppose que $\det(A)\wedge\det(B)=1$. Montrer qu'il existe $U$ et $V$ dans $\mathcal{M}_n(\Z)$ telles que : $$UA+VB=I_n$$
\end{ex}
\begin{proof}
	D'après le théorème de Bézout, on peut fixer $(u,v)\in\Z^2$ tel que $u\det(A)+v\det(B)=1$. Ensuite, on utilise la formule de la comatrice : $$\text{Com}(A)^TA=\det(A)I_n$$
	$$\text{Com}(B)^TB=\det(B)I_n$$ On multiplie ces égalités respectivement par $u$ et $v$ pour obtenir : $$u\text{Com}(A)^TA+v\text{Com}(B)^TB=(u\det(A)+v\det(B))I_n=I_n$$ On pose finalement $U=u\text{Com}(A)^T$ et $V=v\text{Com}(B)^T$, qui sont à coefficients entiers car $u$ et $v$ sont entiers et les comatrices sont à coefficients entiers au vu de leur définition avec les mineurs et les cofacteurs ($\Z$ est un anneau !).
\end{proof}
