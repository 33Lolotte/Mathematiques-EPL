...
\end{ex}
\begin{proof}
	Remarquer que l'application $L$ par excellence est l'intégrale (on construit des mesures à l'aide de telles formes linéaires...). Les évaluations en un point fonctionnent aussi.
	\begin{enumerate}
		\item Montrer l'inégalité $\lvert L(f)\rvert\leqslant L(\lvert f\rvert)$ en passant par les parties positive et négative de $f$. Puis conclure en utilisant le fait que $\lVert f\rVert_\infty-\lvert f\rvert$ est une fonction continue positive. La norme subordonnée est alors $L(1)$ (atteinte pour $f=1$).
		\item Cela ne fonctionne plus : prendre par exemple $L$ qui à $f$ associe $f(1)$ et la suite de fonctions $(x\mapsto x^n)$.
	\end{enumerate}
\end{proof}
...
\begin{ex}[Moulin.Cont.34]{}
	Déterminer, sur $\mathcal{M}_{n,p}(\K)$ identifié canoniquement à $\mathcal{L}(\K^p,\K^n)$, les normes subordonnées aux normes $\lVert\cdot\rVert_\infty$ de $\K^p$ et $\K^n$. Même question pour les normes $\lVert\cdot\rVert_1$. Même question si l'on munit $\K^p$ de la norme $\lVert\cdot\rVert_1$ et $\K^n$ de la norme $\lVert\cdot\rVert_\infty$.
\end{ex}

\begin{ex}[Moulin.Cont.35]{}
	Soit $n\in\N\st$, $\lVert\cdot\rVert_\infty$ la norme du maximum des modules des coefficients sur $\R^n$ et $N$ sa norme subordonnée sur $\mathcal{M}_n(\R)$. On fixe $A=(a_{i,j})_{1\leqslant i,j\leqslant n}\in\mathcal{M}_n(\R)$.
	\begin{enumerate}
...
\begin{ex}[Moulin.Cont.36]{}
	Soit $(E,\lVert\cdot\rVert)$ un espace vectoriel normé et $N$ la norme subordonnée sur $\mathcal{L}_c(E)$.
	\begin{enumerate}
