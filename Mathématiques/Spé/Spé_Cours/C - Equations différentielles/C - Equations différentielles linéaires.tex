...
\begin{thm}{Propriétés du wronskien}%prop
...
\begin{rmq}
	...
	$$F:y\mapsto\left(t\mapsto y_0+\int_{t_0}^{t}\Phi(u,f(u))\d u\right).$$
\end{rmq}
...
\begin{thm}{Existence et unicité des solutions}
...
\begin{exo}
	...
	A quelle condition toutes les solutions sont-elles bornées sur $\R_+$ ?
\end{exo}
...
\begin{itemize}
	...
	\item On peut montrer (sans utiliser le théorème de Cauchy linéaire) que les solutions sur un intervalle $I$ de l'équation différentielle $(E_0)$ sont les fonctions $u_0\cdot y_0$, avec $y_0\in E$ : il suffit pour cela de montrer que toute fonction dérivable $y:I\to E$ est de la forme $u_0(t)\cdot z(t)$ avec $z$ dérivable, et qu'elle est solution si, et seulement si, $z$ est constante (méthode de la variation de la constante).
\end{itemize} 
...
On sait déjà que l'ensemble $\mathcal{S}$ des solutions sur $\R$ de $(E_0)$ est un $\K$-espace vectoriel de dimension $n$.

Résoudre l'équation $(E_0)$ revient donc à déterminer une base de $\mathcal{S}$.
...
Le cas où $A$ est diagonalisable se révèle particulièrement simple : l'ensemble $\mathcal{S}$ des solutions de $(E_0)$ s'obtient directement à partir des éléments propres de $A$.
...
\begin{rmq}
	\begin{itemize}
		...
		$$\mathcal{S}\C=\Vect_\C(\varphi_1,\ldots,\varphi_n)\quad\text{et}\quad\mathcal{S}\R=\Vect_\R(\varphi_1,\ldots,\varphi_n)$$ où la seule différence entre $\mathcal{S}_\C$ et $\mathcal{S}_\R$ réside alors dans les coefficients des combinaisons linéaire (qui sont ou bien complexes ou bien réels).
	\end{itemize}
\end{rmq}
...
\begin{met}
	Si $A\in\mathcal{M}_n(\R)$ est diagonalisable dans $\C$ mais pas dans $\R$, on peut déterminer l'ensemble $\mathcal{S}_\R$ des solutions de $(E_0)$ à partir de la diagonalisation de $A$ :
	\begin{enumerate}
...
\begin{dfn}
	\begin{itemize}
		...
		$$(E)\ :\ x^{(n)}+\sum_{k=0}^{n-1}a_k(t)x^{(k)}=b(t),$$ où $(a_k)_{k\in\llbracket0,n-1\rrbracket}$ est une famille de $n$ applications continues de $I$ dans $\K$, et $b$ est une application continue de $I$ dans $\K$.
		\item L'application $b\in\mathcal{C}^0(I,\K)$ s'appelle le \textbf{second membre} de $(E)$.
		\item On dit que l'équation $(E)$ est à \textbf{coefficients constants} si toutes les fonctions $a_k$ sont constantes et qu'elle est \textbf{homogène}, ou \textbf{sans second membre}, si $b=0$.
		\item On appelle \textbf{équation homogène} associée à $(E)$ l'équation :
		$$(E_0)\ :\ x^{(n)}+\sum_{k=0}^{n-1}a_k(t)x^{(k)}=0.$$
	\end{itemize}
\end{dfn}

\begin{dfn}
	On appelle \textbf{solution} de l'équation différentielle linéaire $(E)$ toute application $n$ fois dérivable $\varphi:I\to\K$ vérifiant :
	$$\forall t\in I,\ \varphi^{(n)}(t)+\sum_{k=0}^{n-1}a_k(t)\varphi^{(k)}(t)=b(t).$$
\end{dfn}

\begin{rmq}
	\begin{itemize}
		\item L'équation $(E)$ dans la première définition est écrite sous une forme dite \textbf{normalisée}, ce qui signifie que le coefficient devant $x^{(n)}$ vaut $1$. Si une équation différentielle linéaire scalaire d'ordre $n$ apparaît sous une forme \textbf{non normalisée}, c'est-à-dire :
		...
		\end{itemize}
		\item Puisque les applications $a_0,\ldots,a_{n-1}$ et $b$ sont \textit{a minima} supposées continues, toute solution de $(E)$ est de classe $\mathcal{C}^n$.
	\end{itemize}
\end{rmq}
...
b(t)
\end{pmatrix}.$$
Les applications continues $A$ et $b$ ainsi définies permettent de traduire l'équation différentielle scalaire $(E)$ sous la forme d'un système différentiel d'ordre $1$.

\begin{thm}{}%prop
	\begin{itemize}
		\item Si une fonction $\varphi\in\mathcal{C}^n(I,\K)$ est solution de l'équation différentielle $(E)$, alors l'application $\Phi=\Gamma(\varphi)\in\mathcal{C}^1(I,\K^n)$ est solution du système différentiel d'ordre $1$ :
		$$(EM)\ :\ X'=A(t)X+B(t).$$
		\item Si $\Phi\in\mathcal{C}^1(I,\K^n)$ est solution du système différentiel $(EM)$, alors sa première fonction coordonnée $\varphi$ est solution de $(E)$.
	\end{itemize}
\end{thm}
\begin{proof}
...
\begin{thm}{Structure de l'ensemble des solutions}%prop
	\begin{enumerate}
		\item L'ensemble $\mathcal{S}_0$ des solutions de l'équation homogène $(E_0)$ est un sous-espace vectoriel de dimension $n$ de $\mathcal{C}^n(I,\K)$.
		\item Si $\varphi_P$ est une solution de $(E)$, alors l'ensemble $\mathcal{S}$ des solutions sur $I$ de l'équation $(E)$ est le sous-espace affine de dimension $n$ de $\mathcal{C}^n(I,\K)$ passant par $\varphi_P$ et de direction $\mathcal{S}_0$ :
		$$\mathcal{S}=\varphi_P+\mathcal{S}_0.$$
	\end{enumerate}
\end{thm}
...
\begin{dfn}[Problème de Cauchy]
	\begin{itemize}
		\item On dit qu'une solution $\varphi$ de l'équation différentielle $(E)$ vérifie la \textbf{condition initiale} $(t_0,x_0,\ldots,x_{n-1})\in I\times\K^n$ si l'on a :
		$$(\varphi(t_0),\ldots,\varphi^{(n-1)}(t_0))=(x_0,\ldots,x_{n-1}).$$
		\item On obtient un \textbf{problème de Cauchy} lorsque l'on munit l'équation $(E)$ d'une condition initiale $(t_0,x_0,\ldots,x_{n-1})\in I\times\K^n$ :
		$$x^{(n)}+\sum_{k=0}^{n-1}a_k(t)x^{(k)}=b(t)\quad\text{et}\quad (\varphi(t_0),\ldots,\varphi^{(n-1)}(t_0))=(x_0,\ldots,x_{n-1}).$$
	\end{itemize}
\end{dfn}

D'après le théorème ci-dessous, un tel problème de Cauchy admet une unique solution. Le résoudre, c'est trouver la solution $\varphi$ de $(E)$ vérifiant la condition initiale

\begin{thm}{Théorème de Cauchy linéaire}%thm
	Pour tout $(t_0,x_0,\ldots,x_{n-1})\in I\times\K^n$, il existe une et une seule solution de (E) vérifiant :
...
Nous étudions dans ce qui suit une équation linéaire scalaire du second ordre de la forme $(E)$ :
$$x''+a_1(t)x+a_0(t)x=b(t)$$
où $a_0$, $a_1$ et $b$ sont des fonctions continues de $I$ dans $\K$. Cette équation est un cas particulier des équations linéaires scalaires d'ordre $n$ étudiées dans la partie précédente. Don :
\begin{itemize}
	\item l'espace vectoriel $\mathcal{S}_0$ des solutions de l'équation homogène $(E_0)$ associée à $(E)$ est de dimension $2$ ;
	\item si $\varphi_P$ est une solution de $(E)$, alors l'ensemble $\mathcal{S}$ des solutions de $(E)$ est le sous-espace affine :
	$$\mathcal{S}=\varphi_P+\mathcal{S}_0\ ;$$
	\item le théorème de Cauchy linéaire assure l'existence et l'unicité d'une solution $\varphi$ de $(E)$ vérifiant une condition initiale de la forme :
...
\begin{ex}
	L'équation différentielle $(E)$ peut se ramener à une équation de la forme $y''+p(t)y=q(t)$.
...
\begin{ex}
	Les zéros (c'est-à-dire les points d'annulation) d'une solution $\varphi$ non nulle de l'équation linéaire scalaire homogène du second ordre $(E)$ :
...
\begin{dfn}
	Soit $\varphi_1$ et $\varphi_2$ deux solutions de l'équation homogène $(E_0)$. \\
...
\begin{ex}
	...
	alors le wronskien $W_{\varphi_1,\varphi_2}$ d'un couple $(\varphi_1,\varphi_2)\in(\mathcal{S}_0)^2$ vérifie $W_{\varphi_1,\varphi2}'=0$ et donc, $I$ étant un intervalle, il est constant sur $I$.
\end{ex}

Le wronskien est un outil qui permet de caractériser les bases de solutions.

\begin{thm}{}%prop
	Soit $\varphi_1$ et $\varphi_2$ des solutions de $(E_0)$. Notons $W$ le wronskien du couple $(\varphi_1,\varphi_2)$.\\
...
\begin{exo}
	...
	Trouver une équation différentielle linéaire homogène d'ordre $2$ dont elles sont solution sur $[0,\pi/2]$.
\end{exo}

\subsection{Recherche d'une solution particulière : méthode de variation des constantes}

Supposons connue une base $(\varphi_1,\varphi_2)$ de l'espace $\mathcal{S}_0$ des solutions de l'équation homogène $(E_0)$. La \textbf{méthode de variation des constantes} consiste alors à chercher une solution particulière de $(E)$ sous la forme :
...
\end{align*}
Ainsi, pour que $\varphi$ soit solution de $(E)$, il suffit d'avoir :
$$\begin{cases}
...
\end{cases}$$
permet de déterminer $\lambda_1'()$ et $\lambda_2'(t)$. Les fonctions $\lambda_1'$ et $\lambda_2'$ sont alors continues comme quotients de fonctions continues. En primitivant, on obtient une solution $\varphi$ de $(E)$.
...
\begin{ex}
	Cherchons les solutions polynomiales de l'équation $(E)$ :
...
\begin{ex}
	Considérons sur $\R$ l'équation homogène $(E_0)$ :
	$$tx''+2x'+tx=0.$$
...
\begin{exo}
	\begin{enumerate}
		...
		$$(E_0)\ :\ (1+t^2)x''+4tx'+2x=0.$$
		On recherchera les solutions développables en série entière.
		\item Résoudre ensuite :
		$$(E)\ :\ (1+t^2)x''+4tx'+2x=\frac{1}{1+t^2}.$$
	\end{enumerate}
\end{exo}

\subsubsection*{Changement de variables}

Dans ce cas, l'équation différentielle $(E)$ se traduit de manière plus simple sur une fonction de la forme $u\mapsto x(\theta(u))$ (idéalement, une équation à coefficients constants). La résolution de cette équation plus simple peut permettre d'en déduire les solutions de cette équation initiale.
...
\begin{ex}
	...
	$$(E)\ :\ t^2x''+3tx'+4x=t\ln(t).$$
	Résolvons $(E)$ grâce au changement de variable $t=e^u$.
\end{ex}

\begin{exo}
	Résoudre sur $\R$ l'équation différentielle :
	$$(E)\ :\ (1+t^2)^2x''+2(1+t)(1+t^2)x'+x=0.$$
...
Supposons connue une solution $\varphi_1$ non nulle de $(E_0)$. Comme $\dim(\mathcal{S}_0)=2$, il reste à déterminer une solution non proportionnelle à $\varphi_1$.
...
\begin{exo}
	...
	\begin{enumerate}
		\item Vérifier que la fonction $\varphi_1:t\mapsto\displaystyle\frac{\sin(t)}{t}$ est solution.
		\item En utilisant le wronskien, terminer la résolution de $(E_0)$.
	\end{enumerate}	
\end{exo}

\subsubsection*{Méthode alternative à la méthode précédente}

Connaissant une solution non nulle $\varphi_1$ de l'équation homogène $(E_0)$, on dispose d'une méthode alternative à la méthode précédente. Elle consiste à rechercher d'autres solutions de $(E_0)$ sous la forme :
...
La fonction $\lambda'$ est solution d'une équation différentielle linéaire d'ordre $1$. Sur un intervalle où $\varphi$ ne s'annule pas, on peut alors déterminer $\lambda$ et obtenir une expression de $\varphi$.
...
Étant donnée une équation différentielle linéaire scalaire de la forme :
$$(E)\ :\ a_n(t)x^{(n)}+\cdot+a_0(t)x=b(t),$$ si la fonction $a_n$ ne s'annule pas sur l'intervalle $I$ de résolution, alors en divisant par $a_n(t)$, on se ramène à une équation normalisée. L'objectif de cette partie est de traiter quelques exemples d'équations ne pouvant être mises sous forme normalisée.
...
\begin{rmq}
	Pour des équations différentielles linéaires non subsistent :
	\begin{itemize}
...
\begin{ex}
	...
	\end{cases}$$
	\begin{enumerate}
		\item Soit $y_1$ une solution réelle non nulle de $(E_1)$ et $y_2$ une solution réelle de $(E_2)$. Si $t_1$ et $t_2$ sont deux zéros distincts de $y_1$, alors il existe $t\in[t_1,t_2]$ tel que $y_2(t)=0$.
...
\begin{met}
	...
	\begin{enumerate}
		...
		\item le lemme de Gronwall peut permettre de montrer que $\Lambda$ est bornée ;
