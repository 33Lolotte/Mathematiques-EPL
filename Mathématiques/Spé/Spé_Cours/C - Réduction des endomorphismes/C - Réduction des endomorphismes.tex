...
\begin{document}
	...
	\end{thm}%prop précédente et pseudo intégrité
	...
	\paragraph{Attention} Lorsque l'on dispose d'un polynôme annulateur $P$ de $u\in\mathcal{L}(E)$, toutes les valeurs propres de $u$ sont racines de $P$, mais certaines racines de $P$ peuvent ne pas être valeur propre de $u$.\\
	...
	\paragraph{Notation} Lorsqu'un endomorphisme $u$ admet un polynôme annulateur non nul, son polynôme minimal est noté $\pi_u$ ou $\mu_u$. De même, le polynôme minimale d'une matrice carrée $A$ est noté $\pi_A$ ou $\mu_A$.
	...
	\paragraph{Attention} La notion de polynôme caractéristique d'un endomorphisme n'a aucun sens si $E$ n'est pas de dimension finie.
	...
	\paragraph{Attention} En dimension infinie, un endomorphisme d'un $\C$-espace vectoriel n'admet pas nécessairement de valeur propre.
	...
	\paragraph{Notation} Dans la suite, l'ordre de multiplicité de $\lambda$ sera noté $m(\lambda)$.
	
	\paragraph{Attention} La notion d'ordre de multiplicité n'a pas de sens en dimension infinie.
	...
	\paragraph{Attention} Comme il fait intervenir le polynôme caractéristique de $u\in\mathcal{L}(E)$, le théorème de Cayley-Hamilton n'a de sens que si $E$ est de dimension \textbf{finie}.
	...
	\paragraph{Attention} Il ne s'agit bien que d'une condition \textit{suffisante} pour que $u$ soit diagonalisable. La proposition suivante donne une caractérisation, qui peut être utile dans le cas où le nombre de valeurs propres est strictement inférieur à la dimension de $E$.
	...
	\paragraph{Terminologie} Un polynôme scindé à racines simples est dit aussi \textbf{simplement scindé}.
	
	\paragraph{Attention} Il faut bien noter que le résultat précédent ne fournit qu'une condition \textit{suffisante} pour que $u$ soit diagonalisable comme le montre, par exemple, le cas d'une homothétie.\\
	...
	\begin{thm}{Trigonalisation simultanée (HP)}%thl
	...
	\paragraph{Terminologie} Les sous-espaces vectoriels $F_\lambda=\ker(u-\lambda\Id_E)^{m(\lambda)}$, pour $\lambda\in\Sp(u)$, sont appelés \textbf{sous-espaces caractéristiques} de l'endomorphisme de $u$.
