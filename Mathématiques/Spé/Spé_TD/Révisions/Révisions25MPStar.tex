...
\begin{exo}[Approximation rationnelle]
	...
	\begin{enumerate}
		...
		\item $\star$ On suppose que $\alpha$ est racine d'un polynôme de degré $n\in\N\st$ à coefficients entiers (on dit que $\alpha$ est \textbf{algébrique}). Montrer qu'il existe un réel $\delta>0$ tel que :
		$$\forall (p,q)\in\Z\times\N\st,\ \left\lvert\alpha-\frac{p}{q}\right\rvert\geqslant\frac{\delta}{q^n}.$$
		\item On pose note $L$ la constante de Liouville :
