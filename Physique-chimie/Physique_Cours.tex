...
\paragraph{Loi de Maxwell-Boltzmann} La loi de Maxwell-Boltzmann décrit un gaz parfait avec $N$ molécules sans interactions. Elle donne la probabilité de mesure une particule avec une vitesse entre $v_x$ et $v_x+dv_x$, $v_y$ et $v_y+dv_y$ et $v_z+dv_z$ par : $$dp_{v_x,v_y,v_z}=C\exp\left(-\frac{mv^2}{2k_BT}\right)dv_xdv_ydv_z$$ où $C$ est une constante de normalisation (qui peut être calculée en se ramenant à l'intégrale de Gauss par changement de variables). Par conséquent, la probabilité de mesurer une particule avec une norme entre $v$ et $v+dv$  est $$dp_v=C\exp\left(-\frac{mv^2}{2k_BT}\right)4\pi v^2dv$$ (coordonnées sphériques pour la vitesse). Cela permet d'obtenir un ordre de grandeur pour $u$ : $u\approx 500$ m.s$\inv$. On a donc $v_\text{son}<u<v_\text{libération}$.
