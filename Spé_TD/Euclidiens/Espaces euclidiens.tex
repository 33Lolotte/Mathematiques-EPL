...
\begin{ex}[Moulin.03]{$\Delta$ Matrices de Gram}
...
\begin{ex}[Moulin.08]{Matrices triangulaires et orthogonales}
...
\begin{ex}[Moulin.09]{Trigonalisation en base orthonormée}
...
\begin{ex}[Moulin.11]{$\Delta$ Décomposition OT}
	\begin{enumerate}
...
\begin{ex}[Moulin.24]{Co-orthodiagonalisation}
...
\begin{ex}[Moulin.32]{Réduction des endomorphismes antisymétriques}
...
\begin{ex}[Moulin.40]{$\star$ Ordre de Löwner}
...
\begin{ex}[Moulin.42]{$\star$ Critère de Jacobi-Sylvester}
...
\begin{ex}[Moulin.47]{$\Delta\star$ Égalités de Courant-Fischer}
...
\begin{ex}[Moulin.48]{$\Delta\star$ Entrelacement des valeurs propres}
...
\begin{ex}[Moulin.49]{$\Delta$ Inégalités de Weyl}
...
\begin{ex}[Moulin.55]{$\Delta$ Transformation de Cayley}
...
\begin{ex}[Moulin.61]{$\star$ Sous-groupes compacts de $\mathcal{GL}_n(\R)$ contenant $\mathcal{O}_n(\R)$}
