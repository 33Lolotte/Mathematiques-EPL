...
\begin{ex}[Moulin.28]{$\Delta$ Sous-groupes à un paramètre de $\mathcal{GL}_n(\C)$}
...
\begin{ex}[Moulin.40 / Cours Moulin]{$\Delta\star$ Lemme de Gronwall}
...
\begin{ex}[Moulin.41]{Lemme de Gronwall : version intégrale}
...
\begin{ex}[Moulin.35]{$\Delta$ Théorème de Floquet}
...
\begin{ex}[Moulin.39]{Équation de Lax}
...
\begin{ex}[Moulin.45]{Au moins une annulation}
...
\begin{ex}[Moulin.47]{Une unique solution périodique}
...
\begin{ex}[Moulin.49]{$\star$} On considère $q\in\mathcal{C}^1(\R_+,\R)$ et l'équation $y''+q(x)y=0$. Alors :
	\begin{itemize}
...
\begin{ex}[Moulin.50]{$\Delta$ Existence de solutions non bornées} Considérons l'équation différentielle $y''+q(x)y=0$ avec $q$ continue et intégrable. Alors :
	\begin{itemize}
