...
\begin{ex}{$\Delta\star$ Théorème de Riesz} Un espace vectoriel normé est de dimension finie si, et seulement si, sa sphère unité est compacte.
\end{ex}
\begin{proof}
	A COMPLETER (cf le cours) Il y a des lemmes à démontrer.
\end{proof}
...
\begin{ex}[Moulin.06]{$\star$ Demi-espaces fermés}
...
\begin{ex}[Moulin.08]{$\Delta$ Enveloppe convexe et théorème de Carathéodory}
	...
	\begin{enumerate}
		...
		\item En déduire le \textbf{théorème de Carathéodory} : si $A$ est une partie d'un $\R$-espace vectoriel de dimension $n$, alors : $$\text{Conv}(A)=\bigcup_{(p_0,\ldots,p_k)\in A^{k+1}}H(p_0,\ldots,p_k)$$
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item L'inclusion réciproque est immédiate. Pour l'inclusion directe, utiliser les hypothèse pour éliminer $p_i$ si $\mu_i\ne 0$.
...
\begin{ex}[Moulin.12]{$\Delta\star$ Théorème de Baire}
...
\begin{ex}[Moulin.13]{Lemme de Croft}
...
\begin{ex}[Moulin.19]{$\star$}
	Montrer qu'une application est continue si, et seulement si, sa restriction à tout compact est continue.
\end{ex}
\begin{proof}
...
\begin{ex}[Moulin.25]{$\Delta$ Application contractante}
...
\end{ex}
\begin{proof}
	Pour l'existence, cette série est absolument convergente ($O(k)$) donc convergente en dimension finie. Donc $f^{n}(x)\to l$. Mais par continuité, $f(l)=l$. Et comme $x$ est fermé, $l\in X$. Pour l'unicité, deux candidats sont nécessairement égaux car l'application est contractante.
\end{proof}

\begin{ex}[Moulin.26]{$\Delta\star$ Théorème de Kakutani} Pour un compact convexe non vide $K$, une application linéaire continue qui stabilise $K$ admet un point fixe.
\end{ex}
\begin{proof}
...
\begin{ex}[Moulin.27]{$\Delta$ Application presque contractante}
...
\begin{ex}[Moulin.29]{$\star$}
	Soit $f:[0,1]\to[0,1]$ une fonction continue et $(x_n)_{n\in\N}$ une suite d'éléments de $[0,1]$ vérifiant $\forall n\in\N,\ x_{n+1}=f(x_n)$. On suppose que tout valeur d'adhérence de $(x_n)_{n\in\N}$ est un point fixe de $f$.
	\begin{enumerate}
...
\begin{ex}[Moulin.31]{$\Delta$ Précompacité} Montrer que si $A$ est compacte, pour tout $\varepsilon>0$, il existe une partie finie $C$ de $A$ telle que $$A\subset\bigcup_{x\in C}B(x,\varepsilon)$$ On dit que $A$ est \textbf{précompacte} lorsqu'elle vérifie la propriété ci-dessus.
\end{ex}
\begin{proof}
...
\begin{ex}[Moulin.33]{Espaces séparables}
...
\begin{ex}[Moulin.34]{Vers la connexité}
...
\begin{ex}[Moulin.43]{$\star$ Complémentaires denses connexes par arcs}
...
\end{proof}
\begin{rmq}
	On peut faire des même dans $\R^2$ en considérant des cercles concentriques de rayons rationnels et irrationnels percé par des demi-droites partant de l'origine.
\end{rmq}

\begin{ex}[Moulin.44]{$\star$}
...
\begin{ex}[Moulin.48]{Presque sous-multiplicativité}
...
\begin{ex}[Moulin.49]{$\Delta$ Compacité de $\mathcal{O}_n(\R)$ et $\mathcal{U}_n(\C)$} $\mathcal{O}_n(\R)$ et $\mathcal{U}_n(\C)$ sont compacts.
\end{ex} \begin{proof}
...
\begin{ex}[Moulin.51]{Matrices de rang fixé non plein}
...
\begin{ex}[Moulin.52]{$\Delta$ Matrices de projecteurs}
...
\begin{ex}[Moulin.53]{Matrices cycliques}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\end{array}$$ qui est continue par multilinéarité en dimension finie. Alors l'ensemble des matrices cycliques est $$\bigcup_{X\in\C^n}\det_X\inv(\C\st)$$ C'est donc un ouvert comme union d'ouverts.
...
\begin{ex}[Moulin.54]{$\star\star$}
	Soit $f:[0,1]\to\mathcal{M}_n(\R)$ continue et $p\in\llbracket0,n\rrbracket$ tels que $$\forall t\in[0,1],\ \text{rg}(M(t))=n-p$$
	\begin{enumerate}
...
\begin{ex}[Moulin.55]{Matrices cycliques version réduction}
...
\begin{ex}[Moulin.56]{Matrices diagonalisables et trigonalisables}
...
\begin{ex}[Moulin.57]{$\star$ Dimension du commutant}
...
\begin{ex}[Moulin.58]{$\Delta$ Racines de l'unité}
...
\begin{ex}[Moulin.59]{Classe de similitude compacte}
