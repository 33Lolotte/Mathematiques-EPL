...
\begin{ex}[Moulin.01]{$\Delta$ Théorème de Darboux}
	...
	\begin{itemize}
		...
		\item l'image direct de n'importe quel sous-intervalle de l'intervalle de départ est un intervalle.
	\end{itemize}
\end{ex}
\begin{proof}
...
\begin{ex}[Moulin.02]{L'image d'un segment par la dérivée est $\R$}
...
\begin{ex}[Moulin.04]{$\Delta$ Classique}
...
\begin{ex}[Moulin.06]{}
	Soit $f$ une application de $\R$ dans $\R_+$ de classe $\mathcal{C}^1$. Montrer qu'il existe une suite $(x_n)_{n\in\N}$ de réels telle que la suite $(f'(x_n))_{n\in\N}$ converge vers $0$.
\end{ex}
...
\end{ex}
\begin{proof}
	Considérer $f:t\mapsto t^x$ sur $\R_+\st$ qui est dérivable de dérive $f':t\mapsto xt^{x-1}$. L'équation se réécrit $f(5)-f(4)=f(3)-f(2)$. Par le théorème des accroissements finis, cela se réécrit $$f'(t_1)(5-4)=f'(t_2)(3-2)$$ pour un certain $2<t_2<3$ et un certain $4<t-1<5$. Cela se réécrit donc $xt_1^{x-1}=xt_2^{x-1}$ donc nécessairement $x=1$ car $t_1\ne t_2$. Et réciproquement, $x=1$ est bien solution.
\end{proof}
...
\begin{ex}[Moulin.08]{$\Delta$ Dérivée des puissances entières}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Écrire $AA\inv=I_n$ et dériver cette relation. On obtient : $$(A\inv)'=-A\inv A' A\inv$$ Pour les puissances, utiliser la question $1$.
	\end{enumerate}
\end{proof}

\begin{ex}[Moulin.09]{$\Delta$ Utilisation d'une série de fonctions}
...
\begin{ex}[Moulin.12]{Dérivation du polynôme caractéristique}
...
\begin{proof}
	\begin{enumerate}
		\item Faire des développements astucieux.
		\item Il correspond au coefficient constant de $\chi_A'$.
	\end{enumerate}
\end{proof}

\begin{ex}[Moulin.13]{Dérivabilité de l'inverse dans une algèbre}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item A faire... Il semblerait que $\varphi$ puisse nous permettre de nous ramener à la question 2.
	\end{enumerate}
\end{proof}

\section{Fonctions de classe $\mathcal{C}^k$}

\begin{ex}[Moulin.14]{$\Delta$ Fonction de décollage en douceur}
...
\begin{ex}[Moulin.15]{Utilisation des fonctions de décollage en douceur}
	Soit $a<b$.
	\begin{enumerate}
...
\begin{ex}[Moulin.18]{$\Delta$ Théorème du relèvement}
	\begin{enumerate}
...
\begin{ex}[Moulin.19]{$\Delta$ Une inégalité d'interpolation}
...
\begin{ex}[Moulin.20]{Construction d'une suite strictement croissante de zéros des dérivées successives}
...
\begin{ex}[Moulin.21]{Formule des rectangles médians}
...
\begin{ex}[Moulin.22]{$\star$ Formule des trapèzes}
...
\begin{ex}[Moulin.23]{Utilisation inhabituelle de Taylor reste intégral}
...
\begin{ex}[Moulin.27]{$\Delta$ Inégalité de Kolmogorov}
...
\begin{ex}[Moulin.28]{Égalité de Taylor-Lagrange}
...
\begin{ex}[Moulin.29]{$\Delta\star$ Théorème de division}
...
\begin{ex}[Moulin.30]{$\star$ Racine carrée d'une fonction paire}
...
\begin{ex}[Moulin.33]{$\Delta$Sous-groupes à un paramètre}
