...
\begin{ex}[Moulin.01]{$\circ$}
	...
	\begin{enumerate}
		...
		\item $\displaystyle\sum 2nnx^{n^2}$ ;
...
\begin{ex}[Moulin.05]{Coefficients binomiaux centraux}
...
\begin{ex}[Moulin.06]{Série hypergéométrique}
...
\begin{ex}[Moulin.09]{Autour du sinus et de $\pi\sqrt{3}$}
...
\begin{ex}[Moulin.11]{$\Delta$ Très important !}
...
\begin{ex}[Moulin.13]{$\star$ Injectivité locale de la somme}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Pour $a=1/7$, on utilise le fait que $a=0,142857142857\ldots$. Avec la convergence absolue, on obtient réécrit la somme en sommant par paquets de 6. Cela donne finalement : $$\sum_{n=1}^{+\infty}a_nx^n=\frac{1}{1-x^6}\left(x+'x^2+2x^38x^4+5x^5+7x^6\right)$$
	\end{itemize}
\end{proof}
\begin{ex}{$\Delta$ Nombre de façons de former $n$ avec $N$ entiers fixés}
	\begin{enumerate}
...
\end{proof}
\begin{ex}{Une technique de filtre}
...
\begin{proof}
	...
	\end{cases}$$ On en déduit $$g_0(t)=\frac{1}{3}(e^{t}+e^{jt}+e^{j^2t})$$ Et par conséquent : $$f(x)=\frac{1}{3}\left(e^{\sqrt[3]{x}}+2e^{-\sqrt[3]{x}/2}\cos\left(\frac{\sqrt{2}}{2}\sqrt[3]{x}\right)\right)$$ De façon générale, on aura une matrice de Vandermonde de taille $p$ à inverser.
\end{proof}
\begin{ex}{$\Delta$ Une intégrale particulière}
...
\begin{ex}{$\Delta$ Fonction bornée DSE sur $\C$}
	Montrer qu'une fonction bornée, somme sur $\C$ d'une série entière, est constante.
\end{ex}
\begin{proof}
	La formule de Cauchy donne avec l'inégalité triangulaire et $M$ une borne de $f$ : $$\forall n\in\N\st,\forall r\in\R_+\st, \ \lvert a_n\rvert\frac{M}{r^n}\xrightarrow[n\to+\infty]{}0$$ Si bien que $\forall n\geqslant 1,\ a_n=0$ donc cette fonction est constante.
\end{proof}
\begin{ex}{$\star$ Principe du maximum}
...
\end{proof}
\begin{ex}{$\Delta$ Fonctions nulles sur un arc de cercle}
...
\begin{ex}{Un calcul d'intégrale par un DSE}
...
\end{proof}
\begin{ex}{Une CNS de contrôle des dérivées pour être DSE}
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
\begin{ex}{$\Delta$ Nombre de partitions}
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
\begin{ex}{$\Delta$ Théorème de Bernstein} Soit $a>0$ et $f\in\mathcal{C}^\infty(]-a,a[,\R)$ telle que $\forall n\in\N,\ \forall x\in]-a,a[,\ f^{(n)}(x)\geqslant 0$. Alors, $f$ est développable en série entière sur $[0,a[$ et sur $]-a,a[$.
\end{ex} \begin{proof}
...
\end{app}
\begin{ex}{$\Delta$ Fonctions analytiques} En utilisant une série double, on montre que si $f$ est DSE sur $I=]-R,R[$, alors pour tout $x_0\in I$, la fonction $t\mapsto f(x_0+t)$ est DSE sur $]-\alpha,\alpha[$ avec $\alpha=R-\lvert x_0\rvert$. On dit que $f$ est analytique sur $I$.
\end{ex}
\begin{proof}
...
\begin{ex}[Moulin.54]{Une série entière dont la somme n'est pas continue}
...
\begin{ex}[Moulin.55]{$\star\star$ Théorème d'Abel non tangentiel}
...
\begin{ex}[Moulin.56]{$\star\star\star$ Théorème de Tauber}
	Si $\sum a_nz^n$ est une série de rayon entière de rayon de convergence $1$, si sa somme $S$ tend vers une limite finie $l$ radialement en $1$ et si $a_n=\displaystyle o\left(\frac{1}{n}\right)$, alors $\sum a_n$ converge et on a : $$\sum_{n=0}^{+\infty} a_n=l$$
\end{ex}
\begin{proof}
...
\section{TD Châteaux}
\begin{ex}{Formule de Cauchy et identité de Parseval}
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
\begin{ex}{Analycité ; principe du maximum et holomorphie}
	Soit $f:z\mapsto\displaystyle\sum_{n=0}^{+\infty}a_nz^n$ la somme d'une série entière de rayon de convergence $R>0$.
	\begin{enumerate}
...
\begin{proof}
	\begin{enumerate}
		\item Pour l'analycité, se référer au cours de Moulin.
	...
	\end{enumerate}
\end{proof}
\begin{ex}{Inverse d'une série entière et une preuve du théorème de d'Alembert-Gauss}
	\begin{enumerate}
	...
	\end{enumerate}
\end{ex}
\begin{ex}{Équivalent au bord de l'intervalle de convergence}
...
\begin{ex}{Théorème de Tauber}
	Soit $\displaystyle\sum a_nx^n$ une série entière de rayon de convergence $1$. On note $f$ sa somme et $S_n=\displaystyle\sum_{k=0}^{n}a_k$. On suppose que $f(x)\xrightarrow[x\to 1^-]{}l$ et $na_n\xrightarrow[n\to+\infty]{}0$. Démontrer que la série $\sum a_n$ converge et que sa somme vaut $l$. Indication : partir de $\displaystyle S_n-f\left(1-\frac{1}{n}\right)$. Donner un contre-exemple en enlevant l'hypothèse $na_n\xrightarrow[n\to+\infty]{}0$.
\end{ex}
\begin{proof}
	Se référer à la section d'exercices qui précède.
\end{proof}
