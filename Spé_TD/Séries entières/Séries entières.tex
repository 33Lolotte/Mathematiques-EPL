...
\begin{ex}[Moulin.17]{Avec un polynôme}
...
\begin{ex}[Moulin.32]{$\star$ Principe du maximum}
...
\begin{ex}[Moulin.33]{$\Delta$ Fonctions nulles sur un arc de cercle}
...
\begin{ex}[Moulin.42]{Une CNS pour être DSE}
...
\begin{ex}[Moulin.43]{$\star$ DSE de $\tan(x)$ et $\tanh(x)$}
	\begin{enumerate}
		...
		\item Montrer : $\forall z\in\C,\ z\rvert<\pi/2\implies S(z)\cos(z)=\sin(z)$.
...
\begin{ex}[Moulin.44]{}
	\begin{enumerate}
		...
		\item $\star$ Montrer que si $\lvert z\rvert<1$ et $1-z=re^{i\theta}$ avec $r>0$ et $\theta_in]-\pi,\pi[$, alors : $$\sum_{n=0}^{+\infty}a_nz^n=\sqrt{r}e^{i\theta/2}$$
	\end{enumerate}
\end{ex}
...
\begin{ex}[Moulin.47]{$\Delta$ Nombre de partitions}
