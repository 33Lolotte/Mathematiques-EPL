...
\end{ex}
\begin{proof}
	La matrice est de rang $2$, symétrique réelle (théorème spectral !) et de trace égale à $1$. Résoudre un système linéaire et utiliser la trace pour trouver les dernières valeurs propres.
\end{proof}

\begin{ex}[Moulin.Red.03]{$\star$}
...
\begin{proof}
	Important !
	\begin{enumerate}
		\item Linéarité à gauche de la composition.
		\item Montrer qu'elles ont le même polynôme minimal (en montrant que les polynômes minimaux en question se divisent mutuellement).
		\item Passer par de la dualité (un hyperplan est le noyau d'une forme linéaire non nulle).
		\item C'est la matrice transposée de celle de $u$.
	\end{enumerate}
\end{proof}

\begin{ex}[Moulin.Red.05]{$\star$}
...
\begin{ex}[Moulin.Red.07]{}
	Quels sont les coefficients des termes de degrés $n-2$ et $1$ du polynôme caractéristique d'une matrice de taille $n$ ?
\end{ex}
\begin{proof}
	Trigonaliser pour se faire une idée et développer le calcul du polynôme caractéristique. Quitte à se placer dans une clôture algébrique, on pourra le faire de toute façon... Et donc cela donne le bon résultat, même si on n'a pas le droit de le faire !
\end{proof}

\begin{ex}[Moulin.Red.08]{$\star$}
...
\begin{ex}[Moulin.Red.16]{$\det(A+N)$ avec commutation et nilpotence}
...
\begin{ex}[Moulin.Red.10]{$\Delta$}
	\begin{enumerate}
		\item $\star$ Soit $(A,B)\in\mathcal{M}_n(\K)^2$. Montrer que $AB$ et $BA$ ont le même polynôme caractéristique. On pourra supposer que $\K$ est un sous-corps de $\C$ et commencer par le cas où l'une des deux matrices est inversible.
...
\end{ex}
\begin{proof}
	Plusieurs méthodes importantes.
	\begin{enumerate}
		\item Raisonner par densité. Le cas inversible est trivial car $AB$ et $BA$ sont alors semblables. Ensuite, raisonner par densité en perturbant légèrement $A$, ou alors plonger $\K$ dans $\K(X)$ !
		\item On obtient la relation $X^p\chi_{AB}=X^n\chi_{BA}$. Pour la trouver, on peut par exemple se référer à un exercice du TD sur les matrices. Sinon, bidouiller avec des matrices inversibles dans les déterminants et utiliser de la densité comme dans le première question. La première question est simplement le cas $p=n$.
		\item En complétant avec des $0$, on se ramène à un calcul de polynôme caractéristique par blocs et on peut utiliser le résultat de la première question.
	\end{enumerate}
\end{proof}

\begin{ex}[Moulin.Red.11]{$\Delta$}
...
\begin{proof}
	\begin{itemize}
		\item $(i)\implies(ii)$ : on écrit $\chi_u=(X-\lambda)^p\times P$. Le théorème de Cayley-Hamilton fournit : $$\text{Im}(u-\lambda\text{id})\subset\ker(P)$$ puis le théorème du rang et le lemme des noyaux fournissent l'égalité et le résultat.
		\item $(ii)\implies(iii)$ : trivial.
		\item $(iii)\implies(i)$ : prendre $F$ un tel supplémentaire. Le polynôme caractéristique de $u$ vaut alors, par blocs : $$\chi_u=(X-\lambda)^{\dim\ker(u-\lambda\text{id})}\times\chi_{u_F}$$ en sachant que $X-\lambda$ ne divise pas $\chi_{u_F}$. Invoquer ensuite l'unicité de la décomposition en facteurs irréductibles pour obtenir, par degré, l'égalité souhaitée.
	\end{itemize}
\end{proof}

\begin{ex}[Moulin.Red.12]{$\Delta$ Trace et nilpotence}
	\begin{enumerate}
...
\begin{proof}
	Grand classique !
	\begin{enumerate}
		\item On définit $P_\sigma$ comme étant $(\delta_{i,\sigma(j)})_{1\leqslant i,j\leqslant n}$. Pour vérifier la propriété de morphisme, raisonner en termes d'applications linéaires car les $P_\sigma$ opèrent une permutation des vecteurs d'une base de $\C^n$.
		\item Il s'agit du nombre de points fixes.
		\item Travailler au préalable sur un seul cycle. Si la permutation est un $n$-cycle, le polynôme caractéristique est $X^n-1$ (facile). Ensuite, réduire par blocs les matrices de permutation en regroupant les vecteurs de la base par cycles, puis calculer le polynôme caractéristique par blocs.
		\item Dans le sens direct, c'est une conséquence immédiate du morphisme de la première question. Dans le sens réciproque, on peut observer que les traces des puissances de ces matrices sont égales. Ensuite, on observe que l'égalité des traces fournit le même nombre de points fixes (\textit{ie} de $1$-cycles), ensuite, l'égalité des traces des carrés fournit l'égalité du nombre de $2$-cycles (en enlevant les $1$-cycles qui sont déjà au même nombre). Par récurrence, les deux permutations ont le même nombre de cycles de même longueur. On conclut avec le lemme classique qui dit que tous les $k$-cycles sont toujours conjugués entre eux dans $\mathcal{S}_n$.
	\end{enumerate}
\end{proof}

\begin{ex}[Révisions]{}
	Soit $A$ et $B$ deux éléments de $\mathcal{M}_n(\C)$. Montrer que les propositions suivantes sont équivalentes :
	\begin{enumerate}[label=(\roman*)]
		\item $\text{Sp}(A)\cap\text{Sp}(B)=\emptyset$
		\item $\chi_A(B)\in\mathcal{GL}_n(\C)$
		\item $\chi_B(A)\in\mathcal{GL}_n(\C)$
	\end{enumerate}
\end{ex}
\begin{proof}
	Considérer $M\mapsto AM-MB$ et montrer qu'elle est injective si l'on suppose la deuxième ou la troisième condition en s'intéressant à des polynômes annulateurs.
\end{proof}

\section{Sous-espaces stables, commutants}

\begin{ex}[Moulin.Red.17]{}
...
\begin{ex}[Moulin.Red.20]{$\Delta$}
	Déterminer le commutant d'une matrice diagonalisable.
\end{ex}
\begin{proof}
	Diagonaliser pour travailler à similitude près (les commutants sont conjugués). On obtient les matrices qui laissent stables tous les sous-espaces propres de $A$.
\end{proof}

\begin{ex}[Moulin.Red.21]{$\star$ Endomorphismes cycliques}
...
\begin{ex}[Moulin.Red.22]{$\Delta$}
	Soit $A_1,\ldots,A_n$ des matrices nilpotentes de $\mathcal{M}_n(\K)$ qui commutent deux à deux. Montre que $A_1A_1\cdots A_n=0$.
\end{ex}
\begin{proof}
	Les noyaux sont stables par commutation. La nilpotence diminue strictement le rang à chaque nouvelle matrice. il est plus facile de le formaliser en termes d'applications linéaires (sinon, cela est rapide en les cotrigonalisant, je dis ça, je dis rien...).
\end{proof}
...
\end{ex}
\begin{proof}
	Les lignes sont de somme constante ! Sommer toutes les colonnes sur la première peut alors s'avérer très utile et fournit pas mal de valeurs propres dans le calcul du polynôme caractéristique.
\end{proof}

\begin{ex}[Moulin.Red.25]{}
...
\end{ex}
\begin{proof}
	Le sens réciproque est immédiat car dans ce cas, $M$ possède $4$ valeurs propres distinctes en dimension $4$. Raisonner par contraposée dans l'autre sens, en montrant que la dimension du sous-espace propre avec une valeur propre double n'est pas suffisante.
\end{proof}

\begin{ex}[Moulin.Red.28]{$\Delta$}
...
\begin{ex}[Moulin.Red.29]{$\Delta$ Matrices de rang $1$}
...
\begin{proof}
	Classique !
	\begin{enumerate}
		\item Observer que toutes les colonnes de $A$ sont proportionnelles à une seule colonne non nulle $X$ puisque $A$ est de rang $1$. Et une colonne est non nulle, donc le $Y$ est non nul.
		\item On remarque que $Y^TX=\Tr(A)$. On en déduit : $$A^2=\Tr(A)A$$ Ainsi, $X(X-\Tr(A))$ annule $A$.
		\item D'après la question 2 et les théorèmes de cours, on en déduit que $A$ est diagonalisable si, et seulement si, $\Tr(A)\ne0$.
		\item Si $\Tr(A)\ne0$, on peut mettre $\Tr(A)$ en $(1,1)$ et des $0$ partout ailleurs. Si $\Tr(A)=0$, on met un $1$ en $(1,2)$ et des $0$ partout ailleurs.
	\end{enumerate}
\end{proof}

\begin{ex}[Moulin.Red.30]{$\star$}
...
\end{ex}
\begin{proof}
	Diagonaliser la matrice de droite pour se ramener à chercher des racines carrées de la matrice diagonale obtenue après diagonalisation (ce qui est beacoup plus simple !). Une matrice de passage et une matrice diagonale qui conviennent sont : $$D=\begin{pmatrix}
		-9&0&0\\
		0&9&0\\
		0&0&0
...
\end{ex}
\begin{proof}
	S'inspirer de l'exercice précédent. En revanche, ici, durant la phase d'analyse, élever la relation au carré permet d'obtenir un polynôme annulateur de $M$ (et donc le spectre, ou presque).
\end{proof}

\begin{ex}[Moulin.Red.32]{}
...
\end{ex}
\begin{proof}
	D'après le cours, cela revient à montrer que la deuxième assertion est équivalente au fait que $\chi_u$ soit scindé et que les dimensions des sous-espaces propres soient égales aux multiplicités des valeurs propres en tant que racines du polynôme caractéristique. Le lemme des noyaux fournit un sens, dans l'autre, fixer un supplémentaire et calculer le polynôme caractéristique par blocs, avant de conclure en invoquant l'unicité de la décomposition en facteurs irréductibles.
\end{proof}

\begin{ex}[Moulin.Red.33]{$\Delta$}
...
\end{ex}
\begin{proof}
	Dans le sens direct, considérer les projecteurs spectraux. Dans le sens réciproque, montrer que les sous-espaces propres sont les images des différents projecteurs et que les valeurs propres associées sont les scalaires présents dans la combinaison linéaire. Vérifier que les dimensions des sous-espaces propres correspondent à la multiplicité des valeurs propres.
\end{proof}

\begin{ex}[Moulin.Red.36]{}
	Déterminer les $A\in\mathcal{M}_n(\C)$ telles que pour tout $P\in\mathcal{GL}_n(\C)$, la matrice $PA$ soit diagonalisable.
\end{ex}
\begin{proof}
	On vérifie le lemme suivant : pour tout $r\in\llbracket1,n\rrbracket$, il existe une matrice de $\mathcal{M}_n(\C)$ de rang $r$ non diagonalisable. Ensuite, si $A$ est de rang $r$ et vérifie les hypothèses de l'énoncé, alors toute matrice de rang $r$ est diagonalisable. En effet, si on fixe $B$ une autre matrice de rang $r$, on peut écrire : $$A=PBQ$$ avec $P$ et $Q$ inversibles. Puis la matrice $Q\inv P\inv PBQ=Q\inv BQ$ est diagonalisable, donc $B$ est diagonalisable. Le lemme vérifié précédemment implique dès lors que $r=0$, donc seule la matrice $A$ peut convenir (et convient effectivement).
\end{proof}

\begin{ex}[Moulin.Red.37]{}
...
\end{ex}
\begin{proof}
	On écrit $A=PDP\inv$ et $B=QD'Q\inv$. On déduit de l'hypothèse, par inversibilité de $P$ et $Q$, que : $$DP\inv MQD'=0$$ On pose $M'=P\inv MQ$, on note $m_{i,j}'$ les coefficients de $M'$, et $d_i$ et $d_i'$ les coefficients respectifs de $D$ et $D'$. Comme $DM'D'=0$, on en déduit : $$\forall (i,j)\in\llbracket1,n\rrbracket^2,\ d_i^rm_{i,j}d_j'^r$$ Comme on travaille dans un corps, on en déduit : $$\forall (i,j)\in\llbracket1,n\rrbracket^2,\ d_im_{i,j}d_j'$$ ce qui donne, en revenant au matrices, $DM'D'=0$, donc $AMB=0$.
\end{proof}

\begin{ex}[Moulin.Red.38]{$\star$}
...
\begin{ex}[Morlot]{}
	Soit $A\in\mathcal{GL}_n(\R)$.
	\begin{enumerate}
		\item Montrer que $A$ est diagonalisable sur $\R$ si, et seulement si, $A-A\inv$ est diagonalisable sur $\R$.
		\item Montrer que $A+A\inv$ peut être diagonalisable sans que $A$ le soit.
	\end{enumerate}
\end{ex}
\begin{proof}
	Posé en colle tous les ans (par le même) !
	\begin{itemize}
		\item Le sens direct est immédiat. pour le sens réciproque, s'intéresser à un polynôme annulateur de $A-A\inv$ scindé à racines simples sur $\R$, et l'utiliser pour construire, après étude de trinômes du second degré, un polynôme annulateur de $A$ scindé à racines simples sur $\R$.
		\item Voir en quoi le raisonnement de la question précédente échoue alors, et s'en servir pour construire un contre-exemple. 
	\end{itemize}
\end{proof}

\begin{ex}[Châteaux]{$\star$ Une extension algébrique de $\Q$}
...
\begin{ex}[Châteaux]{$\Delta\star$}
	...
	\begin{enumerate}
		...
		\item Soit $p$ un nombre premier supérieur ou égal à $3$. On note $\phi$ la projection canonique de $\mathcal{GL}_n(\Z)$ dans $\mathcal{GL}_n(\Z/p\Z)$. Montrer que $\phi$ est un morphisme et que sa restriction à tout sous-groupe fini de l'ensemble de départ est injective.
		\item Peut-on en déduire une majoration sur le cardinal de tels sous-groupes finis ?
	\end{enumerate}
\end{ex}
\begin{proof}
	Tombé aux écrits X-ENS.
	\begin{enumerate}
		\item Classique ! Utiliser la formule de la comatrice dans le sens réciproque, et le fait que le déterminant est polynomial dans l'autre (et les unités de $\Z$).
		\item Le caractère de morphisme de groupes est clair car la projection dans $\Z/p\Z$ est un morphisme d'anneaux. \\ Soit $A$ un matrice d'un sous-groupe fini $G$ de $\mathcal{GL}_n(\Z)$. Alors $A$ est $\C$-diagonalisable et ses valeurs propres sont de module $1$ car elle est annulée par le polynôme $X^{\card(G)}-1$ scindé à racines simples sur $\C$. Supposons que $\phi(A)=I_n$. Alors $A-I_n$ a tous ses coefficients divisibles par $p$, donc $(A-I_n)/p$ est à coefficients entiers. Or, toutes ses valeurs propres complexes sont de module strictement inférieur à $1$, donc comme elle est diagonalisable, en la diagonalisant et en l'élevant à des puissances assez grandes, les puissances de $(A-I_n)/p$ tendent vers $0$. Or, cette matrice est à coefficients entiers donc ses puissances aussi. Comme toute suite d'entiers convergente est stationnaire, on en déduit que $(A-I_n)/p=0$ donc que $A=I_n$ donc $\phi$ est injective (car c'est un morphisme de groupes).
		\item En prenant $p=3$, on en déduit par injectivité de $\phi$ que le cardinal d'un tel groupe est inférieur au cardinal de $\mathcal{GL}_n(\Z/3\Z)$, qui est (grossièrement) inférieur à $3^{n^2}$.
	\end{enumerate}
\end{proof}
...
\begin{proof}
	Classique !
	\begin{enumerate}
		\item Elle est diagonalisable sur $\C$ de valeurs propres appartenant à $\U_n$ car $X^n-1$ scindé à racines simples sur $\C$ annule $A$.
		\item La trace est la somme des valeurs propres et le déterminant est le produit des valeurs propres. Mais ces valeurs sont entières ! Avec l'inégalité triangulaire, on obtient $\Tr(A)\in\{-2,-1,0,1,2\}$ et $\det(A)\in\{-1,1\}$.
		\item Avec la question précédente, on montre que nécessairement, ces valeurs propres sont des racines troisièmes ou quatrièmes de l'unité, si bien que $A^{12}=I_2$. Ensuite, utiliser le fait que l'exposant d'un groupe est inférieur au cardinal du groupe (ou juste le théorème de Lagrange - dans sa version du programme).
	\end{enumerate}
\end{proof}
...
\end{ex}
\begin{proof}
	Commencer par déterminer une CNS sur les coefficients d'une matrice $2\times 2$ pour qu'elle soit diagonalisable (pour le sens réciproque). Pour le sens direct, co-diagonaliser.
\end{proof}
\begin{rmq}
...
\begin{ex}[Moulin.Red.43]{$\star$ Utilisation d'une matrice compagnon}
	\begin{enumerate}
...
\end{ex}
\begin{proof}
	Pour les deux premières questions, se référer à l'exercice précédent. Pour la dernière, considérer la matrice de permutation associée au $n$ cycle $(1\ 2\ \cdots\ n)$.
\end{proof}
...
\end{ex}
\begin{proof}
	Soit $M$ une telle matrice. On déduit de l'hypothèse que pour tout $p\in\N\st$, $M^p$ et $2^pM^p$ sont semblables. Donc pour tout $p\in\N\st$, $\Tr(M^p)=0$ donc $M$ est nilpotente. \\ Réciproquement, soit $M$ une matrice nilpotente. En trigonalisant, on peut prouver à la main que $M$ et $2M$ sont semblables. Sinon, on peut prouver qu'il existe $N\in\mathcal{M}_n(\C)$ telle que $MN-NM=M$. Par récurrence, on en déduit : $$\forall k\in\N,\ MN^k=(I+N)^kM$$ En sommant, on en déduit : $$\forall a\in\C,\ M\exp(aN)=\exp(a)\exp(aN)M$$ et on peut conclure avec $a=\ln(2)$.
\end{proof}
...
\begin{ex}[Châteaux]{Théorème de Burnside}
...
\begin{ex}[Guindy]{$\star$}
	Soit $A\in\mathcal{M}_n(\R)$ telle que $\chi_A\in\Z[X]$. A-t-on nécessairement $\chi_{A^2}\in\Z[X]$ ?
\end{ex}
\begin{proof}
	Oui. Considérer d'une part la matrice compagnon associée à $\chi_A$ et d'autre part, trigonaliser $A$ dans $\C$.
\end{proof}

\begin{ex}[Guindy]{$\star$}
	Soit $p$ un nombre premier ainsi que $a_1,\ldots,a_p$ des entiers. Montrer : $$\begin{vmatrix}
		a_1&a_2&a_3&\cdots&a_p\\
		a_p&a_1&a_2&\cdots&a_{p-1}\\
		a_{p-1}&a_p&a_1&\cdots&a_{p-2}\\
		\vdots&\vdots&\vdots&\ddots&\vdots\\
		a_2&a_3&a_4&\cdots&a_1
	\end{vmatrix}\equiv a_1+\cdots+a_p\ [p]$$
\end{ex}
\begin{proof}
	Considérer $J$ la matrice avec une sur-diagonale de $1$ et un $1$ tout en bas à gauche. Montrer qu'elle est trigonalisable sur $\Z/p\Z$ en utilisant le fait que dans $\Z/p\Z[X]$, on a $X^p-1=(X-1)^p$. Ainsi, $A$ est $\Z/p\Z$-semblable à une matrice triangulaire supérieure dont tous les coefficients diagonaux sont égaux à $a_1+\cdots+a_p$. Utiliser le petit théorème de Fermat pour conclure.
\end{proof}

\section{Polynômes d'endomorphismes}

\begin{ex}[Moulin.Red.47]{}
...
\begin{ex}[Moulin.Red.48]{}
	Soit $f$ un endomorphisme de $\R^3$ tel que $f^3+f=0$ et $\text{rg}f=2$. Déterminer les sous-espaces stables par $f$.
\end{ex}
\begin{proof}
	Réduire $f$ matriciellement par blocs. Observer que $X(X^2+1)$ annule $f$, donc matriciellement, on peut écrire $f$ sous la forme : $$\begin{pmatrix}
		0&1&0\\
		-1&0&0\\
		0&0&0
	\end{pmatrix}$$ Travailler ensuite par blocs.
\end{proof}

\begin{ex}[Moulin.Red.49]{}
	Soit $A$ une matrice inversible. Montrer que $A\inv$ est un polynôme en $A$.
\end{ex}
\begin{proof}
	Il est immédiat que $\text{Vect}(A^k)_{k\in\N}$ est une sous-algèbre. On utilise ensuite le résultat classique selon lequel, pour un élément inversible d'une sous-algèbre de dimension finie, l'inverse est aussi dans la sous-algèbre (pour mémoire, regarder l'endomorphisme "multiplication par $A$" - à gauche par exemple, mais peu importe - prouver l'injectivité avec l'inversibilité, puis utiliser la surjectivité obtenue en dimension finie pour montrer que l'on atteint $I$ et conclure). Donc $A\inv$ appartient à cette sous-algèbre, et est donc un polynôme en $A$. \\ \\ Sinon, on peut passer par le théorème de Cayley-Hamilton pour obtenir directement l'inverse.
\end{proof}

\begin{ex}[Moulin.Red.50]{$\star$}
...
\begin{ex}[Moulin.Red.52]{$\Delta$}
	Montrer que toute matrice de rang $r$ admet un polynôme annulateur de degré $r+1$.
\end{ex}
\begin{proof}
	On note $u$ l'endomorphisme canoniquement associé à une matrice de rang $r$ et $\chi$ le polynôme caractéristique de l'induit de $u$ sur son image. D'après le théorème de Cayley-Hamilton et puisque le rang de la matrice est $r$, $\chi$ est de degré $r$. Puis : $$\forall x,\ \left[\chi \times X\right](u)(x)=\chi(u(x))=0$$ car $u(x)\in\im(u)$. Donc $X\times\chi$, qui est de degré $r+1$, est un polynôme annulateur de $u$, et donc de la matrice.
\end{proof}
...
\end{ex}
\begin{proof}
	Pour se faire une idée, on peut calculer le polynôme caractéristique de $M$. Avec le lemme classique sur les déterminants "blocs $2\times2$" et la commutation, on obtient : $$\chi_M=\chi_A(X-1)\chi_A(X+1)$$ Construire ensuite des vecteurs propres satisfaisant à partir de vecteurs propres de $A$.
\end{proof}

\begin{ex}[Moulin.Red.54]{}
...
\end{ex}
\begin{proof}
	Essayer de calculer des puissances de ces matrices et d'y reconnaître une formule que l'on étend ensuite par linéarité.
\end{proof}

\begin{ex}[Moulin.Red.55]{}
...
\end{ex}
\begin{proof}
	Calculer les puissances $k$-èmes de $B$ pour en déduire par linéarité l'égalité : $$\forall P\in\C[X],\ P(B)=\begin{pmatrix}
		P(A)&(XP')(A)\\
		0&P(A)
	\end{pmatrix}$$ Si $M$ est diagonalisable, alors $M$ est annulée par un $P$ scindé à racines simples, donc $A$ aussi donc $A$ est diagonalisable. Mais $A$ est aussi annulée par $XP'$, donc les valeurs propres de $A$, par degré, sont des racines communes de $P$ et $XP'$, donc sont nécessairement nulles. $A$ est diagonalisable et toutes ses valeurs propres sont nulles, donc $A$ est nulle. La condition suffisante est triviale.
\end{proof}

\begin{ex}[Moulin.Red.56]{$\star$}
	Condition nécessaire et suffisante sur $A\in\mathcal{M}_n(\R)$ pour que $\displaystyle M=\begin{pmatrix}
		0&I_n\\
		A&A
	\end{pmatrix}$ soit $\C$-diagonalisable ? $\R$-diagonalisable ?
\end{ex}
\begin{proof}
	Essayons de calculer $\chi_M$ : avec le lemme classique sur les déterminants par blocs qui commutent, on a : $$\chi_M=\det(XI_n(XI_n-A)-A)=\det(X^2I_n-(X+1)A)=(X+1)^n\chi_A\left(\frac{X^2}{X+1}\right)$$ Ou alors, peut-être essayer de calculer les puissances; Ce sont deux idées, mais je ne sais pas si elles fonctionnent.
\end{proof}

\begin{ex}[Moulin.Red.57]{}
...
\end{ex}
\begin{proof}
	Construire des vecteurs propres par concaténation, ou bien passer par les polynômes annulateurs et les polynômes caractéristiques en utilisant les propriétés du produit de Kronecker vis-à-vis du produit.
\end{proof}

\section{Sous-espaces caractéristiques}

\begin{ex}[Moulin.Red.58]{}
	Soit $u$ un endomorphisme d'un espace vectoriel de dimension finie, et $\lambda$ une valeur propre de $u$ d'ordre $p$. Montrer que les conditions suivantes sont équivalentes :
	\begin{enumerate}[label=(\roman*)]
		\item $p=\dim\ker(u-\lambda\text{id}_E)$ ;
...
\end{ex}
\begin{proof}
	Dans le sens direct, prendre $x_1,\ldots,x_n$ des vecteurs propres associés aux valeurs propres distinctes, puis considérer $x=x_1+\cdots+x_n$, en calculer les puissances puis interpoler pour montrer la liberté. \\
	Dans le sens réciproque, écrire la matrice de $u$ dans cette base pour reconnaître une matrice compagnon.
\end{proof}

\begin{ex}[Moulin.Red.60]{$\star$ Multiplicité dans le polynôme minimal}
...
\begin{ex}[Moulin.Red.61]{}
	...
	\begin{enumerate}
		\item En utilisant la décomposition en sous-espaces caractéristiques, montrer que $v$ est inversible ou nilpotent.
		\item $\star$ Le résultat reste-t-il valable sur un autre corps $\K$ ?
	\end{enumerate}
\end{ex}
\begin{proof}
	Les sous-espaces caractéristiques de $u$ sont stables par $v$ !
\end{proof}

\begin{ex}[Moulin.Red.62]{}
...
\begin{ex}[Moulin.Red.63]{$\Delta$ Décomposition de Dunford}
...
\begin{ex}[Moulin.Red.64]{$\Delta\star$ Décomposition de Jordan}
...
\end{ex}
\begin{proof}
	Une telle racine carrée commute nécessairement avec $A$. $A$ est de rang $1$ et de trace nulle donc n'est pas diagonalisable. Cotrigonaliser $A$ et $B$ dans l'analyse pour obtenir des informations.
\end{proof}

\begin{ex}[Moulin.Red.70]{Une CS de cotrigonalisabilité}
...
\begin{ex}[Châteaux]{Un exemple de cotrigonalisation}
	...
	\end{enumerate}
\end{ex}
\begin{ex}[Moulin.Red.71]{$\Delta$ Crochet de Lie et cotrigonalisation}
