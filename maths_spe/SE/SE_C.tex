...
\addto\captionsfrench{\renewcommand*{\contentsname}{Séries entières}}
...
\begin{prop}[Changement d'indice]%prop
...
\begin{rappel}
	Ces deux résultats restent vrais sans hypothèse de sommabilité pour des familles de réels positifs.
\end{rappel}
...
\begin{lem}[Distributivité généralisée]%lem
...
\begin{attention}
	L'hypothèse de commutation est essentielle !
\end{attention}
...
\begin{notation}
	Soit $z_0\in\K$ et $r\in\overline{\R}_+$.
\begin{itemize}
...
\begin{prop}[Règle de d'Alembert pour les séries entières]%prop
...
\begin{rmq}
	Il n'y a pas de cas d'égalité comme pour la somme. Considérer pour cela $\displaystyle\sum z^n$ et $1-z$.
\end{rmq}
...
\begin{attention}
	La série $\displaystyle\sum a_n z^n$ ne converge pas normalement sur $D_O(0,R)$ en général. Par exemple, la série entière $\displaystyle\sum z^n$ ne converge pas normalement sur $D_O(0,1)$.
\end{attention}
...
\begin{thm}[Théorème d'Abel radial]%thm
...
\begin{attention}
	On ne peut pas \textit{a priori} intégrer sur tout l'intervalle ouvert de convergence, sauf évidemment dans certains cas particuliers, comme par exemple l'exemple ci-dessous.
\end{attention}
...
\begin{prop}[Formule de Cauchy (HP)]%prop
