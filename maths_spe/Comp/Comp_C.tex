...
\addto\captionsfrench{\renewcommand*{\contentsname}{Compacité, connexité, dimension finie}}
...
L'un des objectifs de ce chapitre est de généraliser les trois résultats principaux concernant la continuité des fonctions d'une variable réelle :
\begin{itemize}
...
\begin{dfn}
	On dit qu'une partie $A\subset E$ est \textbf{compacte} si toute suite de $A$ admet une valeur d'adhérence dans $A$, c'est-à-dire si de toute suite de $A$ on peut extraire une suite convergeant dans $A$.
\end{dfn}
...
\begin{met}
	Comme on l'a fait dans l'exemple précédent, une méthode classique pour montrer qu'une partie est non compacte est d'essayer d'en exhiber une suite $(u_n)$ vérifiant $\forall p\ne q,\ d(u_p,u_q)\geqslant\alpha$, où $\alpha$ est un réel strictement positif.
\end{met}
...
\begin{rmq}
	Les normes classiques $\lVert\cdot\rVert_1$ et $\lVert\cdot\rVert_2$ étant équivalentes à la norme $\lVert\cdot\rVert_\infty$, le résultat précédent vaut pour chacune de ces normes, et même pour toutes les normes puisqu'elles sont toutes équivalentes.
\end{rmq}
...
\begin{thm}%thm
	Une suite à valeurs dans un compact est convergente si, et seulement si, elle ne possède qu'une seule valeur d'adhérence.
\end{thm}
...
\begin{exo}
	Montrer que $f:[0,1]^2\to\R$ est continue si, et seulement si, les deux conditions suivantes sont réunies :
	\begin{itemize}
...
\begin{thm}[Théorème des compacts emboîtés]
	Soit $(K_n)_{n\in\N}$ une suite décroissante de compacts tous non vides. Alors $\displaystyle\bigcap_{n\in\N}K_n\ne\emptyset$.
\end{thm}
...
\begin{exo}
	Les résultats présentés dans cet exercice sont en général faux dans un espace métrique ou un espace topologie. Voir l'exercice Moulin.Comp.39
	\begin{enumerate}
...
\begin{exo}
	Montrer que tout ouvert de $\R$ est la réunion au plus dénombrable d'intervalles ouverts disjoints deux à deux.
\end{exo}

\subsection{Image continue d'un connexe par arcs}

On dispose d'une version plus générale du théorème des valeurs intermédiaires.
...
\begin{cor}%cor
	Si $f:E\to\R$ est continue, l'image de tout connexe par arcs de $E$ par $f$ est un intervalle.
\end{cor}
...
\begin{rmq}
	On en déduit par compacité qu'il n'existe pas non plus de bijection continue de $[0,1]$ sur $[0,1]^2$.
\end{rmq}
...
\end{prop}
\begin{proof}
	La fonction indicatrice d'une partie $P$ à la fois ouverte et fermée est continue.
\end{proof}
...
Si $\mathcal{B}=(e_1,\ldots,e_n)$ est une base de $E$, on peut définir la norme $N_\infty$ sur $E$ par : $$N_\infty\left(\displaystyle\sum_{i=1}^{n}x_ie_i\right)=\underset{i\in\llbracket1,n\rrbracket}{\max}\lvert x_i\rvert.$$
...
\begin{rmq}
	Il en résulte qu'en dimension finie, pour les notions de limite, de continuité, de continuité uniforme ou encore d'application lipschitzienne, le choix de la norme est indifférent. Le plus souvent, il n'est alors pas précisé.
\end{rmq}
...
\begin{cor}%cor
	\begin{enumerate}
		...
		\item Toute suite bornée d'un espace vectoriel de dimension finie admettant une unique valeur d'adhérence est convergente (de limite cette unique valeur d'adhérence).
	\end{enumerate}
\end{cor}
...
\begin{thm}%thm
	Soit $E$ un espace vectoriel normé de dimension finie et $F$ un espace vectoriel normé de dimension quelconque. Toute application linéaire de $E$ dans $F$ est continue.
\end{thm}
...
\begin{ex}
	Si $u\in\mathcal{L}(E,F)$, où $E$ est de dimension finie, alors il existe $x\in E$ non nul tel que $\lVert u(x)\rVert=\vertiii{u}\lVert x\rVert$. 
\end{ex}
...
\begin{met}
	\begin{itemize}
		\item $f$ admet une limite finie en $a$ adhérent à $A$ si, et seulement si, $f_i$ admet une limite finie en $a$ pour tout $i\in\llbracket1,n\rrbracket$.
