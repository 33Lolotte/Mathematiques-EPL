...
\addto\captionsfrench{\renewcommand*{\contentsname}{Topologie des espaces vectoriels normés}}
...
\begin{ex}
	\begin{enumerate}
		\item L'application $x\mapsto\lvert x\rvert$ est une norme sur $\K$ (où $\lvert\cdot\rvert$ désigne la valeur absolue si $\K=\R$ et le module si $\K=\C$).
...
\begin{ex}
	De la proposition précédente, on déduit que les espaces préhilbertiens réels sont naturellement munis d'une structure d'espace vectoriel normé. Par exemple :
	\begin{enumerate}
...
\begin{terminologie}
	Dans chacun des deux exemples précédents, la norme euclidienne considérée est appelée \textbf{norme deux}. Tout cela sera précisé dans la section "Exemples d'espaces vectoriels normés".
\end{terminologie}
...
\begin{terminologie}
	Étant donné un vecteur $x$ non nul de $E$, le vecteur $\displaystyle\frac{x}{\lVert x\rVert}$ est appelé \textbf{vecteur unitaire associé à $x$}.
\end{terminologie}
...
\begin{prop}[Inégalités triangulaires]%prop
	Soit $x$ et $y$ deux vecteurs d'un espace vectoriel normé $E$. Alors on a : $$\lvert \lVert x\rVert-\lVert y\rVert\rvert\leqslant\lVert x\pm y\rVert\leqslant\lVert x\rVert+\lVert y\rVert$$
\end{prop}

\begin{terminologie}
	L'inégalité $\lvert \lVert x\rVert-\lVert y\rVert\rvert\leqslant\lVert x\pm y\rVert$ est parfois appelée \textit{seconde inégalité triangulaire}.
\end{terminologie}
...
\begin{attention} 
	Bien comprendre la raison du choix de l'article indéfini \textit{un} dans la phrase "\textit{un} plus court chemin entre deux points" : la ligne droite est un plus court chemin entre deux points, mais ce n'est pas nécessairement le seul (du moins si la norme n'est pas euclidienne ; produire un exemple).
\end{attention}
...
\begin{dfn}
	...
	\begin{enumerate}[label=(\arabic*)]
		...
		\item \textbf{symétrie} : $\forall(x,y)\in A^2,\ d(x,y)=d(y,x)$ ;
...
\begin{ex}%ex fond
	Si $(E,N)$ est un espace vectoriel normé, et si $A$ est une partie de $E$, l'application $(x,y)\mapsto N(x-y)$ définit une distance sur $A$. \\
...
\begin{ex}
	Dans $\C$, on définit $d(a,b)=\lvert b-a\rvert$ si $a$ et $b$ sont positivement colinéaires, et $d(a,b)=\lvert a\rvert+\lvert b\rvert$ sinon. $d$ est une distance sur $\C$, appelée \textbf{distance SNCF}.
\end{ex}

\begin{rmq}%rmq importante
	Toutes les définitions et propriétés qui vont suivre seront formulées dans le cas d'un espace métrique non vide $(E,d)$. Pour rester dans le cadre du programme, il suffit de considérer un espace vectoriel normé $E$ et sa distance $d$ associée.
\end{rmq}
...
\begin{notation} 
	Dans $\C$, on parle plutôt de \textit{disque}, et l'on utilise alors les notations $D_o(a,r)$ et $D_f(a,r)$ au lieu de $B_o(a,r)$ et $B_f(a,r)$ respectivement.
\end{notation}

\begin{rmq}
	Il est clair, d'après la définition, que les boules dépendent de la distance considérée sur $E$ : une partie de $E$ peut être une boule pour une distance et ne pas l'être pour une autre distance.
\end{rmq}
...
\begin{terminologie}
	Étant donnés deux points $x$ et $y$,  l'ensemble des points de la forme $$(1-\lambda)x+\lambda y\ \ \text{avec}\ \ \lambda\in[0,1]$$ est appelé \textbf{segment} $[x,y]$.
\end{terminologie}
...
\begin{prop}[Stabilité par passage au barycentre à coefficients positifs (HP)]%prop
...
\begin{prop}%prop
	Toute boule (ouverte ou fermée) d'un espace vectoriel normé est une partie convexe.
\end{prop}
...
Le résultat suivant sera utile pour prouver l'homogénéité d'une norme définie par une borne supérieure.

"Dans sa grande sagesse, le programme nous donne ce lemme. Pour une fois que je peux mettre sagesse et programme dans la même phrase..."
...
\begin{ex}
	Cela s'applique en particulier à l'espace vectoriel $\mathcal{B}(\N,\K)$ des suites bornées à valeurs dans $\K$. Pour $(u_n)_{n\in\N}\in\K^\N$, on a : $$\mathcal{N}_\infty(u)=\underset{n\in\N}{\sup}\lvert u_n\rvert.$$
\end{ex}
...
$$\lVert P\rVert_1=\sum_{n=0}^{+\infty}\lvert a_n\rvert\ \ \lVert P\rVert_2=\sqrt{\sum_{n=0}^{+\infty}\lvert a_n\rvert^2}\ \ \text{et}\ \ \lVert P\rVert_\infty=\underset{n\in\N}{\sup}\lvert a_n\rvert$$
...
\begin{ex}
	\begin{enumerate}
		...
		\item Soit $(a,b)\in\R^2$ avec $a<b$. Notons $E=\mathcal{C}([a,b],\K)$.\\
		Si $\omega:[a,b]\to\K$ est une fonction continue et à valeurs strictement positives, alors l'application : $$\begin{array}{lrcl}
			...
			&f&\longmapsto&\displaystyle\int_{a}^{b}\omega(x)\lvert f(x)\rvert \d x
...
\begin{rmq}
	Plus généralement, si $(E_1,d_1),\ldots,(E_p,d_p)$ sont des espaces métriques, on peut définir la \textbf{distance produit} sur $E_1\times\cdots\times E_p$ en posant : $$d((x_1,\ldots,x_p),(y_1,\ldots,y_p))=\max(d_1(x_1,y_1),\ldots,d_p(x_p,y_p)).$$
\end{rmq}
...
\begin{itemize}
	...
	\item Comme le prouve le cas de la distance de $0$ à $\R_+\st$, il n'existe pas toujours d'élément $a\in A$ tel que $d(x,A)=d(x,a)$. Lorsqu'un tel élément existe, on dit que la distance de $x$ à $A$ est \textbf{atteinte}.
\end{itemize}
\end{attention}
...
\begin{dfn}
	Le \textbf{diamètre} d'une partie $A$ de $E$ est défini dans $\overline{\R}_+$ par : $$\text{diam}(A)=\sup\{d(x,y)\ \mid\ (x,y)\in A^2\}$$ si $A$ est non vide, et $0$ sinon.\\
...
\begin{prop}[Caractérisation des parties bornées]%prop
...
\begin{notation}
	Pour signifier qu'une suite $(a_n)_{n\in\N}$ tend vers $l$, on écrit $a_n\xrightarrow[n\to+\infty]{}l$ ou plus simplement $a_n\to l$.
\end{notation}

\begin{prop}[Unicité de la limite]%prop
...
\begin{dfn}
	Une suite $(a_n)_{n\in\N}$ est dite \textbf{convergente} s'il existe un élément $l\in E$ tel que $a_n\to l$. Cet unique élément $l$ est alors appelé \textbf{limite} de la suite $(a_n)_{n\in\N}$, et on le note $\lim a_n$. \\
	Une suite qui n'est pas convergente est dite \textbf{divergente}.
\end{dfn}

\begin{notation}
	Pour dire qu'une suite est convergente (respectivement divergente), on dit souvent simplement qu'elle converge (respectivement diverge).
\end{notation}

\begin{attention} 
	La convergence d'une suite à valeurs dans un espace métrique dépend par définition de la distance avec laquelle on travaille. Une suite peut en effet converger pour une distance et diverger pour une autre.\\
...
\begin{rmq}
	Nous verrons cependant que deux normes équivalentes définissent les mêmes suites convergentes. En particulier, en dimension finie, la convergence et la limite éventuelle d'une suite ne dépendant pas de la norme choisie.
\end{rmq}
...
\begin{prop}[Combinaison linéaire de deux suites convergentes]%prop
...
\begin{prop}[Caractérisation des valeurs d'adhérence]%prop
...
\begin{notation} 
	On note $S_n(u)$ la $n$-ième somme partielle de la série de terme général $u_n$. Lorsque la série converge, on note aussi $S(u)$ sa somme.
\end{notation}
...
\begin{rmq}
	\begin{itemize}
		...
		\item Le caractère asymptotique de la notion de limite permet de voir que pour $n_0\in\N$, une série $\displaystyle\sum u_n$ converge si, et seulement si, la série $\displaystyle\sum_{n\geqslant n_0}u_n$ converge.
	\end{itemize}
\end{rmq}
...
\begin{dfn}
	Une série dont le terme général ne tend pas vers $0$ est dite \textbf{grossièrement divergente}.
\end{dfn}

\begin{attention} 
Comme pour les séries numériques, la convergence vers $0$ du terme général est une condition nécessaire mais non suffisante de convergence.
\end{attention}
...
\begin{attention}
	Contrairement à ce qui se passe dans $\R$ ou dans $\C$, une série absolument convergente peut très bien ne pas converger.
\end{attention}
...
\begin{proof}
	Pour $x$ appartenant à la boule ouverte de centre $a$ et de rayon $R$, considérer la boule ouverte de centre $x$ et de rayon $R-d(x,a)$. Faire un dessin peu aider...
\end{proof}
...
\begin{attention}
	Le caractère ouvert ou non d'un ensemble n'est pas une notion intrinsèque à cet ensemble, mais dépend de l'espace dans lequel on le considère. Si $F$ est un sous-espace métrique de $E$, alors une partie ouverte dans $F$ n'est en général pas une partie ouverte dans $E$, comme le montre l'exercice suivant.
\end{attention}
...
\begin{exo}
	Montrer que toute partie ouverte peut s'écrire comme une réunion de boules ouvertes.
\end{exo}

\begin{attention}
	Avant d'affirmer qu'une intersection d'ouverts est un ouvert, il faut bien vérifier le caractère fini de cette intersection. En effet, une intersection quelconque de parties ouvertes n'est en général pas ouverte, comme l'illustre l'exemple suivant.
\end{attention}

\begin{exo}
	Pour tout $n\in\N\st$, on note $U_n=\displaystyle\left]-\frac{1}{n},\frac{1}{n}\right[$. Montrer que, bien que chacun des $U_n$ soit un ouvert, leur intersection $\{0\}$ ne l'est pas.
\end{exo}
...
\begin{attention}
Il ne faut pas croire, par jeu de mots, qu'une partie qui n'est pas ouvert est nécessairement fermée. En effet, dans tout $\K$-espace vectoriel normé non réduit à $\{0\}$, il existe des parties ni ouvertes ni fermées.\\
...
\begin{attention}
	Avant d'affirmer qu'une réunion de fermés est fermée, il faut bien vérifier le caractère fini de cette réunion. En effet, une réunion quelconque de fermés n'est en général pas fermée, comme l'illustre l'exercice suivant.
\end{attention}
...
\begin{rmq}
	On peut, sans en changer le sens, remplacer "boule ouverte" par "boule fermée" dans la définition qui précède.
\end{rmq}
...
\begin{dfn}
	\begin{enumerate}
		...
		On note $\mathcal{V}_\infty$ l'ensemble des voisinages de l'infini.
...
\begin{rmq}
	\begin{itemize}
		...
		\item Dans $\R$, a-t-on $\mathcal{V}_\infty=\mathcal{V}_{+\infty}\cap\mathcal{V}_{-\infty}$ $\mathcal{V}_\infty=\mathcal{V}_{+\infty}\cup\mathcal{V}_{-\infty}$ ?
		\end{itemize}
\end{rmq}
...
\begin{prop}[Écriture topologique des limites]%prop
	Une suite $(u_n)_{n\in\N}$ tend vers $l$ si, et seulement si, l'on a : $$\forall V\in\mathcal{V}_l,\ \exists n_0\in\N\ : \ n\geqslant n_0\implies u_n\in V,$$ ceci étant valable pour $l\in E$ ou $l=\infty$ lorsque $E$ est un espace vectoriel normé, et pour $l\in\R$ ou $l=\pm\infty$ lorsque $E=\R$.
\end{prop}
...
\begin{itemize}
	...
	\item L'intérieur d'une intersection finie est contenu dans l'intersection des intérieurs, avec égalité pour une intersection finie.
\end{itemize}

\begin{exo}
	Montrer qu'une partie $A$ est ouverte si, et seulement si,  $\mathring{A}=A$.
\end{exo}
...
\begin{ex}
	\begin{enumerate}
		...
		\item Borne supérieure d'une partie de $\R$.
	\end{enumerate}
\end{ex}
...
\begin{prop}[Caractérisation séquentielle des parties fermées]
...
\begin{ex}
	\begin{enumerate}
		\item Soit $E$ un espace vectoriel normé non trivial. Si $B$ désigne la boule fermée de centre $a\in E$ et de rayon $r>0$, alors l'intérieur de $B$ est la boule ouverte de mêmes centre et rayon, et la frontière de $B$ est alors la sphère de mêmes centre et rayon.
...
\begin{exo}
	Soit $A$ une partie de $E$. Montrer : $\text{Fr}(A)=\overline{A}\cap\overline{(E\setminus A)}$.
\end{exo}
...
\begin{terminologie}
	Si une topologie $\mathcal{O}$ est la topologie associée à une distance $d$, on dit qu'elle est \textbf{métrisable.}
\end{terminologie}

\begin{ex}
	La topologie grossière est non métrisable si $E$ possède plus de éléments. En effet, il est aisé  de montrer qu'alors, toute suite d'éléments de $E$ converge vers tout élément de $E$. Si $E$ possède au moins deux éléments, il n'y a donc pas unicité de la limite, si bien que la topologie ne provient pas d'une distance.
\end{ex}
...
\begin{terminologie}
	Pour signifier que $N_1$ est dominée par $N_2$, on dit aussi que $N_2$ est \textbf{plus fine} que $N_1$.
\end{terminologie}
...
\begin{met}
	...
	\begin{itemize}
		...
		\item est bornée pour $N_2$ mais ne l'est pas pour $N_1$.
	\end{itemize}
\end{met}
...
Beaucoup de propriétés sont conservées lorsqu'on passe d'une norme à une autre qui lui est équivalente. \\
...
\begin{prop}[Conservation du caractère borné d'une partie]%prop
...
\begin{prop}[Conservation du caractère convergent d'une suite]
...
\begin{prop}[Conservation des ouverts et des fermés]%prop
	Soit $N_1$ et $N_2$ deux normes équivalentes sur $E$, et $A$ une partie de $E$. Alors :
	\begin{itemize}
		\item la partie $A$ est ouverte pour $N_1$ si, et seulement si, elle l'est pour $N_2$ ;
...
\begin{terminologie}
	Comparer des normes $N_1$ et $N_2$ signifie déterminer si $N_1$ (respectivement $N_2$) est dominée par $N_2$ (respectivement $N_1$).
\end{terminologie}
