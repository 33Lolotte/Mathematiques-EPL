...
\addto\captionsfrench{\renewcommand*{\contentsname}{Endomorphismes d'un espace euclidien}}
...
\begin{ex}
	\begin{enumerate}
		\item L'application : $$\begin{array}{rcl}
			...
			(x,y)&\longmapsto&\displaystyle\sum_{i=1}^{n}x_iy_i
		\end{array}$$ est un produit scalaire sur $\R^n$, appelé \textbf{produit scalaire canonique de $\R^n$}.
		\item Plus généralement, si $E$ est muni d'une base $(e_i)_{i\in I}$, on peut définir un produit scalaire sur $E$ en posant : $$\forall (x_i)\in\R^{(I)},\ \forall (y_i)\in\R^{(I)},\ \left(\sum_{i\in I}x_ie_i\bigg| \sum_{i\in I} y_ie_i\right)=\sum_{i\in I}x_iy_i.$$
		...
		\item Soit $[a,b]$ un segment de $\R$ d'intérieur non vide. L'application : $$\varphi:(f,g)\mapsto\int_{a}^{b}f(x)g(x)\ \d x$$ définit une forme bilinéaire symétrique positive sur $\mathcal{CM}([a,b],\R)$. C'est un produit scalaire sur $E=\mathcal{C}^0([a,b],\R)$.
	\end{enumerate}
\end{ex}
...
\paragraph{Projection sur un convexe fermé (HP)} L'identité du parallélogramme, et surtout sa reformulation, l'identité de la médiane : $$\lVert x\rVert^2+\lVert y\rVert^2=\frac{1}{2}\lVert x-y\rVert^2+2\left\lVert\frac{x+y}{2}\right\rVert^2,$$ sont essentielles dans la démonstration du très important théorème de projection sur un convexe fermé dans les espaces de Hilbert. Il est parfois donné en exercice d'oral.
...
\begin{exo}
	Montrer les propriétés suivantes de l'orthogonal :
	\begin{enumerate}
		\item $A\bot A^\bot$.
...
\begin{rmq}
	L'ensemble des vecteurs isotropes est stable par dilatation. C'est ce que l'on appelle un \textbf{cône}. On parle donc parfois de \textbf{cône isotrope}.
\end{rmq}
...
\begin{dfn}
	\begin{itemize}
		...
		\item On appelle \textbf{famille orthonormée} (ou \textbf{orthonormale}) de $E$ toute famille de vecteurs de $E$ normés et deux à deux orthogonaux.
	\end{itemize}
\end{dfn}
...
\begin{exo}
	Soit $O(t)$ un point ; $\overrightarrow{u}(t)$, $\overrightarrow{v}(t)$ deux vecteurs orthonormés de l'espace, et $M(t)$ un point du plan $(O(t),\overrightarrow{u}(t),\overrightarrow{v}(t))$. On suppose que les fonctions $O$, $\overrightarrow{u}$, $\overrightarrow{v}$ et $M$ sont continues. Montrer que les coordonnées de $M(t)$ dans $(O(t),\overrightarrow{u}(t),\overrightarrow{v}(t))$ sont des fonctions continues de $t$.
\end{exo}
...
\begin{ex}
	Calcul de $\displaystyle\underset{(a,b)\in\R^2}{\inf}\int_{0}^{2\pi}(x-a\cos(x)-b\sin(x))^2\ \d x$.
\end{ex}
...
\begin{exo}
	...
	\begin{enumerate}
		...
		\item Retrouver le fait que si $F$ est de dimension finie, alors $E=F\oplus F^\bot$.
	\end{enumerate}
\end{exo}
...
\begin{enumerate}
	...
	\item Montrer que $\rg(G(x_i))=\rg (x_i)$.
	...
	\item Montrer que si $F$ est un sous-espace vectoriel dont $(y_1,\ldots, y_p)$ est une base, on a la formule : $$d(x,F)^2=\frac{\text{Gram}(x,y_1,\ldots,y_p)}{\text{Gram}(y_1,\ldots,y_p)}.$$
...
\begin{ex}
	\begin{enumerate}
		...
		\item En reprenant l'exemple précédent, l'endomorphisme $D^2-2MD$ est égal à son adjoint. %polynômes orthogonaux
	\end{enumerate}
\end{ex}
...
\begin{cor}
	Soit $u\in\mathcal{L}(E)$. Alors on a : $$\det(u\st)=\det(u)\ ; \ \Tr(u\st)=\Tr(u)\ \text{et}\ \rg(u\st)=\rg(u).$$
\end{cor}

\begin{prop}
	Soit $u\in\mathcal{L}(E)$. Alors on a : $$\ker(u\st)=\text{Im}(u)^\bot \ \ \text{et} \ \ \text{Im}(u\st)=\ker(u)^\bot.$$
\end{prop}
%l'un des deux est vrai en dimension quelconque, pour l'autre, seule une inclusion est valable en dimension quelconque, l'autre s'obtient par la dimension finie
...
\begin{lem}
	Soit $u\in\mathcal{L}(E)$ et $F$ un sous-espace vectoriel de $E$ stable par $u$. Alors $F^\bot$ est stable par $u\st$.
\end{lem}
...
\begin{ex}
	\begin{enumerate}
		...
		\item La base canonique de $\R^n$ étant une base orthonormée, tout matrice obtenue en permutant les colonnes et/ou les lignes de la matrice $I_n$ est orthogonale. Ces matrices représentent des éléments de $\mathfrak{S}_n$.
...
\begin{rmq}
	On munit $\R^n$ de sa structure euclidienne canonique, si bien que la base canonique est une base orthonormée. Soit $A\in\mathcal{M}_n(\R)$. L'endomorphisme canoniquement associé à la matrice $A$ est autoadjoint si, et seulement si, $A\in\mathcal{S}_n(\R)$.
\end{rmq}
...
\begin{prop}
	Soit $u\in\mathcal{S}(E)$ et $F$ sous-espace vectoriel stable par $u$. Alors $F^\bot$ est stable par $u$.
\end{prop}
...
\begin{exo}
	\begin{enumerate}
		\item Montrer que pour tout $u\in\mathcal{S}(E)$, on a $\vertiii{u}=\displaystyle\underset{\lambda\in\text{Sp}(u)}{\max}\lvert\lambda\rvert$.
		\item Montrer que pour tout $u\in\mathcal{L}(E)$, on a $\vertiii{u}=\sqrt{\vertiii{u\st\circ u}}=\displaystyle\underset{\lambda\in\text{Sp}(u\st\circ u)}{\max}\sqrt{\lambda}$.
	\end{enumerate}
\end{exo}
...
\begin{rmq}
	Soit $A\in\mathcal{S}_n(\R)$. $A$ représente une forme quadratique sur $\R^n$. Si on note $B$ la sous-matrice de $A$ obtenue en conservant les coefficients $(i,j)\in\llbracket1,n-1\rrbracket^2$, alors $B$ représente la restriction de la forme quadratique à $\R^{n-1}$.
\end{rmq}
...
\begin{dfn}
	...
	\begin{itemize}
		...
		\item \textbf{symétrique définie positive} lorsque : $$\forall x\in\R^n\setminus\{0\},\ (x\mid Ax)> 0\ \ \text{ou encore}\ \ \text{Sp}(A)\subset\R_+\st.$$
	\end{itemize}
\end{dfn}

\begin{notation}
	On note respectivement $\mathcal{S}_n^+(\R)$ et $\mathcal{S}_n^{++}(\R)$ l'ensemble des matrices symétriques positives et l'ensemble des matrices symétriques définies positives.
\end{notation}
...
\begin{exo}
	Démontrer le théorème de pseudo-réduction simultanée en utilisant des racines carrées.
\end{exo}
...
\begin{prop}
	Soit $(x,y,z)\in E^3$ un triplet de vecteurs non nuls. Alors, on a : $$\widehat{(x,z)}\equiv\widehat{(x,y)}+\widehat{(y,z)}\ [2\pi].$$
\end{prop}
...
\begin{conséquence}
	On munit $\R^3$ de sa structure euclidienne orientée canonique et on se donne $A$ dans $\mathcal{SO}_3(\R)$. Alors il existe $\theta\in\R$ tel que $A$ soit orthogonalement semblable à une matrice de la forme $$\begin{pmatrix}
	1&0&0\\
	0&\cos(\theta)&-\sin(\theta)\\
	0&\sin(\theta)&\cos(\theta)
\end{pmatrix}.$$
