...
\begin{thm}{Formules de polarisation}
...
\begin{thm}{Identité du parallélogramme}
...
\begin{thm}{}
	Soit $F$ un sous-espace vectoriel de dimension fini d'un espace préhilbertien réel $E$ et $x$ un vecteur de $E$. La distance du vecteur $x$ à $F$ est atteinte en un unique point de $F$, à savoir $\pi_F(x)$. Autrement dit : 
	\begin{enumerate}
...
\begin{ex}
	On munit $\R[X]$ du produit scalaire définie par $$(P\mid Q)=\int_{-\infty}^{+\infty}P(x)Q(x)e^{-x^2}\ dx$$ En notant $D$ l'opérateur de dérivation et $M$ celui de multiplication par $X$, on a $M\st=M$ et $D\st=2M-D$.
\end{ex}
...
\paragraph{Notation} On note $\mathcal{O}_n(\R)$ ou encore $\mathcal{O}(n)$ l'ensemble des matrices orthogonales de $\mathcal{M}_n(\R)$.
...
\paragraph{Notation} On note $\mathcal{SO}_n(\R)$ l'ensemble des matrices orthogonales positives de $\mathcal{M}_n(\R)$, aussi appelées \textbf{matrices de rotations}.

\begin{thm}{}
	Les ensembles $\mathcal{O}_n(\R)$ et $\mathcal{SO}_n(\R)$ sont des sous-groupes de $\mathcal{GL}_n(\R)$.
\end{thm}

\paragraph{Terminologie} Ces groupes sont respectivement appelés \textbf{groupe orthogonale d'ordre n} et \textbf{groupe spécial orthogonal d'ordre n}.

\subsection{Espaces euclidiens orientés}

Dans cette sous-section, on suppose que $E$ est un espace euclidien non nul.

\paragraph{Rappel} On définit sur l'ensemble des bases de $E$ une relation d'équivalence en posant :
...
\paragraph{Terminologie} Pour $(u_1,\ldots,u_n)\in E^n$, le réel $\det_\mathcal{B}(u_1,\ldots,u_n)$, indépendant de la base orthonormée directe $\mathcal{B}$ choisie, est appelé \textbf{produit mixte} de $(u_1,\ldots,u_n)$ et parfois noté $[u_1,\ldots,u_n]$.
...
\paragraph{Notation} L'ensemble des endomorphismes autoadjoints de $E$ est noté $\mathcal{S}(E)$.

\paragraph{Terminologie} Un endomorphisme autoadjoint est parfois appelé \textbf{endomorphisme symétrique}, ce qui explique la notation $\mathcal{S}(E)$.
...
\paragraph{Attention} L'ensemble $\mathcal{S}(E)$ n'est en général pas stable par composition.
...
\paragraph{Attention} Ce résultat n'est plus vrai pour les matrices complexes. La matrice complexe symétrique $\displaystyle\begin{pmatrix}
	i&1\\
	1&-i
...
\paragraph{Notation} L'ensemble des endomorphismes autoadjoints positifs (respectivement définis positifs) de $E$ est noté $\mathcal{S}^+(E)$ (respectivement $\mathcal{S}^{++}(E)$).
...
\paragraph{Notation} On note respectivement $\mathcal{S_n}^+(\R)$ et $\mathcal{S}_n^{++}(\R)$ l'ensemble des matrices symétriques positives et l'ensemble des matrices symétriques définies positives.
...
\paragraph{Notation} On note $\mathcal{O}(E)$ l'ensemble des isométries vectorielles de $E$.
...
\paragraph{Terminologie} Une isométrie étant un élément de $\mathcal{GL}(E)$, on parle parfois d'\textbf{automorphisme orthogonal} pour désigner un tel élément, ce qui explique la notation $\mathcal{O}(E)$.
...
\paragraph{Notation} On note $\mathcal{SO}(E)$ l'ensemble des isométries positives de $E$, aussi appelées \textbf{rotations} de $E$.
...
\paragraph{Conséquence} On munit $\R^n$ de sa structure euclidienne orientée canonique. Soit $A\in\mathcal{M}_n(\R)$. La matrice $A$ est orthogonale (respectivement orthogonale positive) si, et seulement si, l'endomorphisme canoniquement associé à $A$ est une isométrie (respectivement une isométrie directe).

\begin{thm}{}
	Les ensembles $\mathcal{O}(E)$ et $\mathcal{SO}(E)$ sont des sous-groupes de $\mathcal{GL}(E)$.
\end{thm}

\paragraph{Terminologie} Les groupes $\mathcal{O}(E)$ et $\mathcal{SO}(E)$ sont appelés respectivement \textbf{groupe orthogonal de E} et \textbf{groupe spécial orthogonal de E}.
...
\paragraph{Terminologie} Une symétrie orthogonale par rapport à un hyperplan est appelée \textbf{réflexion}.
...
\paragraph{Terminologie} Un tel élément de $\mathcal{SO}(E)$ est appelé \textbf{rotation d'angle $\theta$}. Le réel $\theta$ est appelé la \textbf{mesure de l'angle de la rotation}.
...
\paragraph{Notation} L'angle orienté des vecteurs non nuls $x$ et $y$ est noté $(x,y)$.
...
\paragraph{Conséquence} Toute matrice orthogonale est orthogonalement semblable à une matrice de la forme précédente.
...
\paragraph{Conséquence} On munit $\R^3$ de sa structure euclidienne orientée canonique et on se donne $A\in\mathcal{SO}_3(\R)$. Alors il existe $\theta\in\R$ tel que $A$ soit orthogonalement semblable à une matrice de la forme $$\begin{pmatrix}
	1&0&0\\
	0&\cos(\theta)&-\sin(\theta)\\
	0&\sin(\theta)&\cos(\theta)
\end{pmatrix}$$
